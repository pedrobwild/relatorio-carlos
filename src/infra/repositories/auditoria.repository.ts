...
import { supabase, executeListQuery, getPaginationRange, type PaginationParams } from './base.repository';
 ...
 */
export async function listAudits(filters: AuditoriaFilters = {}): Promise<AuditoriaListResult> {
...
export function formatAuditsForCSV(audits: AuditoriaWithUser[]): string {
  const headers = ['Data/Hora', 'Usuário', 'Ação', 'Entidade', 'ID Entidade', 'Obra ID'];
  const rows = audits.map(audit => [
    new Date(audit.created_at).toLocaleString('pt-BR'),
    audit.users_profile?.nome || audit.users_profile?.email || audit.por_user_id || 'Sistema',
