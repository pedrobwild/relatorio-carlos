...
export async function checkRlsBasics(userId: string): Promise<RlsCheckResult> {
  ...
  try {
    ...
    checks.push({
      ...
      details: projectsError ? projectsError.message : `${projectsData?.length || 0} projeto(s) acessível(is)`,
    });
    ...
    checks.push({
      ...
      passed: !countError,
      details: countError ? countError.message : `${count || 0} projeto(s) visíveis (filtrado por RLS)`,
    });
    ...
    checks.push({
      ...
      passed: !profileError,
      details: profileError ? profileError.message : (profileData ? `Role: ${profileData.role}` : 'Perfil não encontrado'),
    });
    ...
    return {
      ...
      message: allPassed ? 'Todos os checks de RLS passaram' : 'Alguns checks de RLS falharam',
      checks,
    };
  } catch (err) {
    return {
    ...
    if (error) {
      ...
      if (error.message.includes('not found') || error.message.includes('Object not found')) {
        return {
          ...
          details: 'Arquivo de teste não existe, mas função responde corretamente',
        };
      }
...
export async function runFullDiagnostics(): Promise<{
  ...
  const rls = auth.userId ? await checkRlsBasics(auth.userId) : {
