...
describe('validateFile', () => {
  ...
  it('rejects files exceeding size limit', () => {
    const file = createMockFile('large.pdf', 'application/pdf', MAX_FILE_SIZE_BYTES + 1);
    const result = validateFile(file);
  ...
  it('accepts file at exactly the size limit', () => {
    const file = createMockFile('exact.pdf', 'application/pdf', MAX_FILE_SIZE_BYTES);
    const result = validateFile(file);
    expect(result.valid).toBe(true);
  });
});
...
describe('sanitizeFilename', () => {
  ...
  it('replaces special characters with underscores', () => {
    ...
    expect(sanitizeFilename('file@name#special.pdf')).toBe('file_name_special.pdf');
  });
...
describe('ALLOWED_MIME_TYPES', () => {
  ...
  it('includes common image types', () => {
    const requiredTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
