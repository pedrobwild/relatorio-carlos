...
export function DocumentViewer({
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (isPanning && scrollContainerRef.current) {
      scrollContainerRef.current.scrollLeft =
        scrollStart.x - (e.clientX - panStart.x);
      scrollContainerRef.current.scrollTop =
        scrollStart.y - (e.clientY - panStart.y);
    }
  };
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (isPanning && scrollContainerRef.current && scale > 1) {
      scrollContainerRef.current.scrollLeft =
        scrollStart.x - (e.touches[0].clientX - panStart.x);
      scrollContainerRef.current.scrollTop =
        scrollStart.y - (e.touches[0].clientY - panStart.y);
    }
  };
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-52">
              ...
              <DropdownMenuItem onClick={resetZoom} className="min-h-[44px]">
                <RotateCcw className="w-4 h-4 mr-2" /> Zoom{" "}
