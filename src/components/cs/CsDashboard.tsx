...
interface CsDashboardProps {
  ...
  onFilter?: (filter: { status?: CsTicketStatus | null; severity?: CsTicketSeverity | null }) => void;
}
...
function KpiTile({ icon: Icon, label, value, hint, accent = 'default', onClick }: KpiTileProps) {
  const accentMap = {
    default: { bg: 'bg-muted/40', icon: 'text-muted-foreground', value: 'text-foreground' },
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        onClick && 'hover:border-primary/40 hover:shadow-sm active:scale-[0.99] cursor-pointer',
      )}
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className={cn('text-2xl font-bold leading-none mt-1 tabular-nums', cfg.value)}>{value}</p>
    ...
    <div className="space-y-4 mb-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-3">
        ...
        <KpiTile
          ...
          accent="destructive"
          hint={stats.altosAtivos > 0 ? `+${stats.altosAtivos} altos` : undefined}
              ...
              <table className="w-full text-sm border-separate border-spacing-1">
                ...
                <tbody>
                  {statusOrder.map((st) => {
                    ...
                    const rowTotal = severityOrder.reduce((acc, sev) => acc + stats.matrix[st][sev], 0);
                    return (
                      <tr key={st}>
                          ...
                          return (
                            <td key={sev} className="p-0">
                              <button
                                type="button"
                                disabled={count === 0}
                                onClick={() => onFilter?.({ status: st, severity: sev })}
                                className={cn(
        ...
        <section className="space-y-4">
          ...
          <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4">
            <header className="flex items-center gap-2 mb-2.5">
              ...
              <h3 className="text-sm font-semibold text-destructive">Críticos para resolver</h3>
