...
import { Lightbulb, AlertTriangle, TrendingUp, Activity, Target, GitCompareArrows, BarChart3 } from"lucide-react";
...
export function InsightCard({ insight, className }: { insight: Insight; className?: string }) {
 ...
 <p className="text-[10px] uppercase tracking-wide text-muted-foreground truncate">{e.label}</p>
 <p className="text-sm font-semibold truncate" title={String(e.value)}>
 {e.value}
 </p>
 </div>
 ))}
 ...
 <span className="font-semibold">Ação recomendada:</span> {insight.recommendedAction}
 </div>
 )}
 </CardContent>
 </Card>
