...
const statusConfig: Record<string, { label: string; icon: string; color: string }> = {
 draft: { label:'Rascunho', icon:'✎', color:'bg-violet-500/10 text-violet-600 border-violet-500/20' },
 active: { label:'Em andamento', icon:'●', color:'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' },
 completed: { label:'Concluída', icon:'✓', color:'bg-blue-500/10 text-blue-600 border-blue-500/20' },
 paused: { label:'Pausada', icon:'‖', color:'bg-amber-500/10 text-amber-600 border-amber-500/20' },
...
export function WorkQuickPreviewDrawer({ project, summary, open, onOpenChange }: WorkQuickPreviewDrawerProps) {
 ...
 blockers.push({ icon: <FileSignature className="h-3.5 w-3.5" />, label:`${summary.unsigned_formalizations} assinatura(s) pendente(s)`, accent:'text-amber-600' });
 }
 if (summary && summary.pending_documents > 0) {
 blockers.push({ icon: <FileText className="h-3.5 w-3.5" />, label:`${summary.pending_documents} documento(s) pendente(s)`, accent:'text-amber-600' });
 }
...
function MiniStat({ label, value, accent ='default' }: { label: string; value: number | string; accent?:'default' |'warning' |'destructive' }) {
 const colors = { default:'text-foreground', warning:'text-amber-600', destructive:'text-destructive' };
 return (
