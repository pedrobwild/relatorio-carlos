...
const kpiDefinitions: KpiDefinition[] = [
  ...
  },
  {
    key: "draft",
    label: "Rascunhos",
    description: "Obras em rascunho aguardando finalização do cadastro",
  ...
  },
  {
    ...
    description: "Obras com data de entrega ultrapassada",
    icon: <CalendarX className="h-4 w-4" />,
    accent: "destructive",
  },
  {
    key: "approaching-deadline",
    label: "Entrega próxima",
    description: "Entrega nos próximos 14 dias",
  ...
  {
    key: "cost-at-risk",
    label: "Custo em risco",
    description: "Obras com desvio de custo acima de 15%",
    icon: <DollarSign className="h-4 w-4" />,
    accent: "destructive",
  },
  {
    key: "critical-purchase",
    label: "Compra crítica",
    description: "Obras com compra crítica pendente",
  ...
  for (const p of projects) {
    ...
    if (p.status === "active" && financials) {
      ...
      if (fin && fin.budget_approved > 0) {
        if (
          (fin.cost_committed + fin.cost_realized) / fin.budget_approved - 1 >
          0.15
        ) {
          costAtRisk++;
        }
...
export function PortfolioKpiStrip({
  ...
  const values = useMemo(
    () => computeKpiValues(projects, summaries, financials),
    [projects, summaries, financials],
  );
      ...
      {kpiDefinitions
        ...
        .map((kpi) => {
          ...
          const shouldHighlight =
            (kpi.key === "overdue" || kpi.key === "critical") &&
            !isZero &&
            !isSelected;
