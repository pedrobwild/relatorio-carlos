...
export function MobileProjectList({ projects, onProjectClick }: MobileProjectListProps) {
  ...
  if (isLoading) {
    return (
      <div className="space-y-2">
        {[1, 2, 3, 4].map(i => (
          <div key={i} className="h-[88px] bg-card rounded-xl border border-border/40 animate-pulse" />
        ))}
      </div>
    );
  }
