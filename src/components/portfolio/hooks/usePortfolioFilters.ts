...
export function usePortfolioFilters(
  ...
  const filtered = useMemo(() => {
    ...
    const wantsDraft =
      kpiFilter === "draft" || advancedFilters.status.includes("draft");
    if (!wantsDraft) {
    ...
    if (selectedEngineer) {
      result = result.filter(
        (p) => (p.engineer_user_id ?? p.engineer_name) === selectedEngineer,
      );
    }
      ...
      case "due-soon":
        result = result.filter((p) => {
          ...
          const daysLeft = Math.ceil(
            (new Date(p.planned_end_date).getTime() - Date.now()) /
              (1000 * 60 * 60 * 24),
          );
    ...
    // KPI
    if (kpiFilter)
      result = applyKpiFilter(result, summaries, kpiFilter, financials);
