...
import {
 Ban, ClipboardX, CalendarX, Clock,
...
const urgencyConfig: Record<Urgency, { badge: string; dot: string; iconBg: string }> = {
 critical: {
...
export function PortfolioActionInbox({ projects, summaries, onNavigate }: PortfolioActionInboxProps) {
 ...
 <Inbox className="h-3 w-3 text-muted-foreground/60" aria-hidden="true" />
 <h3 className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70">
 Ações
 </h3>
 {items.length > 0 && (
 <Badge variant="secondary" className="h-4 px-1 text-[9px] font-bold tabular-nums bg-muted">
 {items.length}
 </Badge>
 )}
 </div>
 {remaining > 0 && (
 <Button variant="ghost" size="sm" className="h-5 text-[10px] text-primary gap-0.5 px-1.5">
 ...
 <p className="text-[10px] text-muted-foreground mt-0.5">Nenhuma ação urgente</p>
 </div>
 ) : (
...
'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring',
 ...
 <div className={cn('flex items-center justify-center h-6 w-6 rounded-md', cfg.iconBg)}>
 {item.icon}
 </div>
 {item.urgency ==='critical' && (
 <div className={cn('absolute -top-0.5 -right-0.5 h-1.5 w-1.5 rounded-full border border-card', cfg.dot,'animate-pulse')} />
 )}
 </div>
