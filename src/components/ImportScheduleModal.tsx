...
import { generateActivitiesFromTemplate, type ActivityTemplateSet } from '@/data/activityTemplates';
import { type ActivityFormData, type ColumnMapping, type ImportStep, REQUIRED_FIELDS, FIELD_LABELS } from './import-schedule/types';
...
export const ImportScheduleModal = ({ open, onOpenChange, onImport }: ImportScheduleModalProps) => {
  ...
  const [columnMapping, setColumnMapping] = useState<ColumnMapping>({
    description: '', plannedStart: '', plannedEnd: '', actualStart: '', actualEnd: '', weight: '',
  });
  ...
  const resetState = () => {
    ...
    setRawData([]);
    setHeaders([]);
    setColumnMapping({ description: '', plannedStart: '', plannedEnd: '', actualStart: '', actualEnd: '', weight: '' });
    setMappedData([]);
  };
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (fileName.endsWith('.csv')) {
        ...
        data = XLSX.utils.sheet_to_json(firstSheet, { raw: false, dateNF: 'yyyy-mm-dd' }) as Record<string, string>[];
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        {step === 'upload' && (
          <UploadStep onFileUpload={handleFileUpload} onTemplateSelect={handleTemplateSelect} isProcessing={isProcessing} />
        )}
        {step === 'mapping' && (
          <MappingStep
            headers={headers}
        ...
        {step === 'preview' && (
          <PreviewStep mappedData={mappedData} onBack={() => setStep('mapping')} onImport={handleImport} />
        )}
      </DialogContent>
    </Dialog>
  );
