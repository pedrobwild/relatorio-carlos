...
const typeConfig: Record<string, { icon: typeof Bell; className: string; actionVerb: string }> = {
  payment_due: { icon: DollarSign, className: "text-warning", actionVerb: "Pagar parcela" },
  payment_overdue: { icon: DollarSign, className: "text-destructive", actionVerb: "Regularizar pagamento" },
  formalization_pending: { icon: ClipboardSignature, className: "text-primary", actionVerb: "Assinar formalização" },
  document_uploaded: { icon: FileText, className: "text-success", actionVerb: "Revisar documento" },
  stage_changed: { icon: Layers, className: "text-primary", actionVerb: "Ver nova etapa" },
  pending_item_created: { icon: AlertCircle, className: "text-warning", actionVerb: "Resolver pendência" },
  report_published: { icon: TrendingUp, className: "text-success", actionVerb: "Ver relatório" },
  general: { icon: Info, className: "text-muted-foreground", actionVerb: "Ver detalhe" },
};
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-[10px] text-muted-foreground/70 mt-1">{formatTime(notification.created_at)}</p>
      </div>
      {isUnread && (
        <span className={cn("w-2.5 h-2.5 rounded-full shrink-0 mt-1.5", isBlocking ? "bg-destructive animate-pulse" : "bg-primary")} />
      )}
    </button>
  );
}
...
export function MobileNotificationsSheet({ open, onOpenChange }: MobileNotificationsSheetProps) {
  ...
  const displayed = activeTab === "actions"
    ? actionNotifications
    : activeTab === "updates"
      ? updateNotifications
      : notifications;
