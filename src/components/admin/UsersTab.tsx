    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          {([null, 'admin', 'engineer', 'manager', 'customer'] as const).map((r) => (
            <Button key={r ?? 'all'} variant={roleFilter === r ? 'default' : 'outline'} size="sm" onClick={() => setRoleFilter(r)}>
              {r === null ? 'Todos' : r === 'admin' ? 'Admins' : r === 'engineer' ? 'Engenheiros' : r === 'manager' ? 'Gestores' : 'Clientes'}
            </Button>
          ))}
        </div>
      </div>
        ...
        <>
          ...
          <div className="sm:hidden space-y-3">
            {filteredUsers.map((user) => (
              <UserCard key={user.id} user={user} onRoleChange={handleRoleChange} onDelete={handleDelete} onEdit={handleEdit} onResetPassword={handleResetPassword} />
            ))}
          </div>
              ...
              <TableBody>
                {filteredUsers.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell><RoleSelector user={user} onRoleChange={handleRoleChange} /></TableCell>
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        <EditUserDialog user={user} onSave={handleEdit} />
                        <ResetPasswordDialog user={user} onReset={handleResetPassword} />
                        <DeleteUserAlert email={user.email} onDelete={() => handleDelete(user.id)} />
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
