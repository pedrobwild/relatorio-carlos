...
import { Plus, Trash2, GripVertical, Pencil, ClipboardCheck } from 'lucide-react';
...
export function InspectionTemplatesTab() {
  ...
  const [form, setForm] = useState({ category: '', description: '', sort_order: 0, is_active: true });
  ...
  const upsertMutation = useMutation({
    mutationFn: async (params: { id?: string; category: string; description: string; sort_order: number; is_active: boolean }) => {
      if (params.id) {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-inspection-templates'] });
  ...
  const toggleActive = useMutation({
    mutationFn: async ({ id, is_active }: { id: string; is_active: boolean }) => {
      const { error } = await supabase.from('inspection_templates').update({ is_active }).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-inspection-templates'] });
  ...
  const openEdit = (t: InspectionTemplate) => {
    setEditingId(t.id);
    setForm({ category: t.category, description: t.description, sort_order: t.sort_order, is_active: t.is_active });
    setDialogOpen(true);
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Itens padrão para vistorias, organizados por categoria.</p>
        </div>
        ...
        categories.map(cat => (
          <Card key={cat}>
            ...
            <CardContent className="space-y-1">
              {grouped[cat].map(t => (
                <div key={t.id} className="flex items-center justify-between py-1.5 px-2 rounded hover:bg-muted/50 group">
                  <div className="flex items-center gap-2 min-w-0">
                    <GripVertical className="h-3.5 w-3.5 text-muted-foreground/40 shrink-0" />
                    <span className={`text-sm truncate ${!t.is_active ? 'line-through text-muted-foreground' : ''}`}>
                      {t.description}
                    </span>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingId ? 'Editar Item' : 'Novo Item de Checklist'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={!form.category.trim() || !form.description.trim() || upsertMutation.isPending}>
