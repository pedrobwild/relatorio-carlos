...
export function useTemplatesTabState() {
  ...
  const filtered = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      ...
      if (sortBy === "is_project_phase")
        return (a.is_project_phase ? 0 : 1) - (b.is_project_phase ? 0 : 1);
  ...
  const resolveActivities = (f: FormState): ActivityItem[] => {
    if (
      f.selected_activity_template === "__none__" ||
      !f.selected_activity_template
    )
      return [];
    if (f.selected_activity_template === "__custom__")
      return f.custom_activities;
    const preset = activityTemplateSets.find(
      (a) => a.id === f.selected_activity_template,
    );
  ...
  const openEdit = (t: ProjectTemplate) => {
    ...
    setForm({
      ...
      selected_activity_template:
        actMatch?.id ?? (activities.length > 0 ? "__custom__" : "__none__"),
    ...
    if (presetId === "__custom__") {
      ...
      setForm((p) => ({
        ...
        custom_activities:
          current.length > 0
            ? current
            : [{ description: "", durationDays: 5, weight: 10 }],
      }));
    } else {
      setForm((p) => ({
        ...p,
  ...
  const addActivity = () => {
    setForm((p) => ({
      ...
      custom_activities: [
        ...p.custom_activities,
        { description: "", durationDays: 5, weight: 5 },
      ],
    }));
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      default_contract_value: form.default_contract_value
        ? parseFloat(form.default_contract_value)
        : null,
        ...
        for (const item of items) {
          ...
          await createTemplate.mutateAsync({
            ...
            default_contract_value:
              typeof item.default_contract_value === "number"
                ? item.default_contract_value
                : null,
            category:
              typeof item.category === "string" ? item.category : "geral",
          });
          count++;
        }
        toast({
