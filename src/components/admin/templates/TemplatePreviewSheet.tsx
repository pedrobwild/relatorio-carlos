...
import { type ActivityItem, totalDays, totalWeight, getCategoryLabel } from './types';
...
export function TemplatePreviewSheet({
  template, onClose, versions, restoreVersion, onEdit, onExport, onDuplicate, toast,
        ...
        <div className="space-y-4 mt-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="rounded-lg border p-3">
              ...
              <p className="font-medium text-sm">{getCategoryLabel(template.category || 'geral')}</p>
            </div>
            <div className="rounded-lg border p-3">
              ...
              <>
                ...
                <div className="rounded-lg border p-3 bg-muted/30">
                  ...
                  <div className="space-y-1">
                    {(() => {
                      const acts = template.default_activities as ActivityItem[];
                  ...
                  <div className="flex justify-between mt-1.5">
                    ...
                    <span className="text-[10px] text-muted-foreground">Dia {totalDays(template.default_activities as ActivityItem[])}</span>
                  </div>
                </div>
                {/* Table */}
                <div className="rounded-lg border overflow-hidden">
                  <Table>
                    ...
                    <TableBody>
                      {(template.default_activities as ActivityItem[]).map((act, i) => (
                        <TableRow key={i}>
                      ...
                      <TableRow className="bg-muted/50 font-medium">
                        ...
                        <TableCell className="text-sm text-right">
                          {totalDays(template.default_activities as ActivityItem[])}d
                        </TableCell>
                        <TableCell className="text-sm text-right">
                          {totalWeight(template.default_activities as ActivityItem[])}%
                        </TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
                  ...
                  {versions.map((v) => (
                    <div key={v.id} className="flex items-center justify-between rounded-lg border p-2.5">
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            await restoreVersion.mutateAsync({ templateId: template.id, version: v });
          ...
          <div className="flex gap-2 pt-2">
            <Button onClick={() => { onClose(); onEdit(template); }} className="flex-1 gap-2">
              <Pencil className="h-4 w-4" /> Editar
            </Button>
            <Button variant="outline" onClick={() => onExport(template)} className="gap-2">
              <Download className="h-4 w-4" /> Exportar
            </Button>
            <Button variant="outline" onClick={() => { onClose(); onDuplicate(template); }} className="gap-2">
