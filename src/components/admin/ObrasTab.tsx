...
export function ObrasTab() {
  ...
  const filteredProjects = useMemo(
    () =>
      projects.filter((p: ProjectWithCustomer) => {
        ...
        const matchesEngineer =
          !engineerFilter || p.engineer_user_id === engineerFilter;
  ...
  const { draftCount, activeCount, completedCount, pausedCount } = useMemo(
    () => ({
      draftCount: projects.filter(
        (p: ProjectWithCustomer) => p.status === "draft",
      ).length,
      activeCount: projects.filter(
        (p: ProjectWithCustomer) => p.status === "active",
      ).length,
      completedCount: projects.filter(
        (p: ProjectWithCustomer) => p.status === "completed",
      ).length,
      pausedCount: projects.filter(
        (p: ProjectWithCustomer) => p.status === "paused",
      ).length,
    }),
    [projects],
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        <Card className="p-4">
          ...
          <p className="text-h2 font-bold text-muted-foreground">
            {pausedCount}
          </p>
        </Card>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        {engineers.length > 0 && (
          <Select
            ...
            onValueChange={(v) => setEngineerFilter(v === "all" ? null : v)}
          >
                        ...
                        >
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <Button
                              ...
                              size="icon"
                              onClick={() =>
                                navigate(`/gestao/obra/${project.id}`)
                              }
                            >
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ...
                                    <strong>{project.name}</strong> e todos os
                                    dados associados serão permanentemente
                                    removidos.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
