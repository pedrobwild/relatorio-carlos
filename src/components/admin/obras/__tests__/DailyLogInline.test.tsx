...
vi.mock("@/hooks/useProjectDailyLog", async () => {
  const actual = await vi.importActual<
    typeof import("@/hooks/useProjectDailyLog")
  >("@/hooks/useProjectDailyLog");
  return {
    ...actual,
    useProjectDailyLog: (
      ...args: Parameters<typeof actual.useProjectDailyLog>
    ) => mockUseProjectDailyLog(...args),
    useSaveProjectDailyLog: (
      ...args: Parameters<typeof actual.useSaveProjectDailyLog>
...
function makeLog(partial: Partial<ProjectDailyLog> = {}): ProjectDailyLog {
  return {
    id: "log-1",
    project_id: "proj-1",
    log_date: "2026-04-27",
...
describe("DailyLogInline — seção unificada Serviços e prestadores", () => {
  ...
  it("mostra microcopy de estado vazio quando não há serviços nem prestadores", async () => {
    ...
    expect(screen.getByText(/Nenhum serviço adicionado/i)).toBeInTheDocument();
    expect(
