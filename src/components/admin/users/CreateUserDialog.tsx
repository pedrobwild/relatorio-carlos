...
import { type IdentifierType, type ProjectOption, formatCPF, isValidCPF, cpfToEmail } from './types';
...
export function CreateUserDialog({ onUserCreated }: { onUserCreated: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!role) {
      toast({ title: 'Erro', description: 'Selecione uma permissão', variant: 'destructive' });
      return;
    }
    if (!password || password.length < 6) {
      toast({ title: 'Erro', description: password ? 'A senha deve ter pelo menos 6 caracteres' : 'Senha é obrigatória', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      toast({ title: 'Erro', description: err instanceof Error ? err.message : 'Não foi possível criar o usuário', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Preencha os dados para criar uma nova conta de usuário.</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label>Tipo de Identificação *</Label>
              <Select value={identifierType} onValueChange={(v) => { setIdentifierType(v as IdentifierType); setIdentifier(''); }}>
            ...
            <div className="space-y-2">
              <Label htmlFor="identifier">{identifierType === 'email' ? 'Email' : 'CPF'} *</Label>
              <Input id="identifier" type="text" placeholder={identifierType === 'email' ? 'usuario@exemplo.com' : '000.000.000-00'} value={identifier} onChange={(e) => handleIdentifierChange(e.target.value)} required />
              {identifierType === 'cpf' && <p className="text-xs text-muted-foreground">O usuário fará login usando o CPF como identificador</p>}
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="displayName" type="text" placeholder="Nome do usuário" value={displayName} onChange={(e) => setDisplayName(e.target.value)} />
            </div>
            <div className="space-y-2">
            ...
            {role === 'customer' && (
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground mb-2">Selecione as obras que este usuário poderá visualizar</p>
                {loadingProjects ? (
                  <div className="flex items-center justify-center p-4"><Loader2 className="h-4 w-4 animate-spin" /></div>
                ) : projects.length === 0 ? (
                  <p className="text-sm text-muted-foreground p-2 text-center">Nenhuma obra disponível</p>
                ) : (
                  <ScrollArea className="h-[150px] rounded-md border p-2">
                    <div className="space-y-2">
                      {projects.map((project) => (
                        <label key={project.id} className="flex items-start gap-2 p-2 rounded hover:bg-muted cursor-pointer">
                ...
                {selectedProjects.length > 0 && <p className="text-xs text-muted-foreground">{selectedProjects.length} obra(s) selecionada(s)</p>}
              </div>
            )}
          </div>
          <DialogFooter>
