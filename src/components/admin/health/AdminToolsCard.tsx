...
import { FileText, Trash2, Bug, ExternalLink, AlertTriangle } from 'lucide-react';
...
export const AdminToolsCard = ({ state, roles, isAdmin }: AdminToolsCardProps) => {
    ...
    <Card>
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Button variant="outline" className="h-auto py-4 flex-col gap-2" onClick={copyDiagnosticReport}>
            <FileText className="w-5 h-5" />
            <span>Copiar Relatório</span>
            <span className="text-xs text-muted-foreground">JSON para suporte</span>
          </Button>

          <Button variant="outline" className="h-auto py-4 flex-col gap-2" onClick={clearLocalCaches}>
          ...
          {(isDev() || buildInfo.environment !== 'production') && (
            <Button variant="outline" className="h-auto py-4 flex-col gap-2" onClick={emitTestError}>
          ...
          {sentryUrl ? (
            <Button variant="outline" className="h-auto py-4 flex-col gap-2" onClick={() => window.open(sentryUrl, '_blank')}>
