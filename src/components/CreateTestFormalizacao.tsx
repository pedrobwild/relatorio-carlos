    ...
    try {
      ...
      const { data: formalization, error: formalizationError } = await supabase
        ...
        .insert({
          ...
          summary: 'Substituição do porcelanato previsto por mármore conforme solicitação.',
...
Cliente solicitou upgrade do revestimento.`,
      ...
      if (formalizationError) {
        ...
        toast.error('Erro ao criar formalização: ' + formalizationError.message);
        return;
      }
      ...
      const { error: customerPartyError } = await supabase
        ...
        .insert({
          ...
          party_type: 'customer',
          display_name: profile.display_name || user.email?.split('@')[0] || 'Cliente',
    ...
    <Card className="border-dashed">
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-xs">
          Crie uma formalização real no banco de dados para testar o fluxo de assinatura
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Button 
