...
export function GanttDebugColumn({ ganttTasks, getBarStyle }: GanttDebugColumnProps) {
  return (
    <div className="flex-shrink-0 w-72 border-r border-border bg-amber-50/50 overflow-hidden">
      <div className="h-8 border-b border-border bg-amber-100/50 flex items-center px-2">
        <span className="text-[10px] font-mono font-bold text-amber-800">🐛 DEBUG: Datas & Estilos</span>
      </div>
      {ganttTasks.map((task, index) => {
        const plannedStyle = getBarStyle(task.plannedStart, task.plannedEnd);
        const actualStyle = task.statusTabela !== 'PENDENTE' ? getBarStyle(task.start, task.end) : null;

        return (
          <div key={`debug-${index}`} className="h-12 px-2 py-1 border-b border-amber-200 text-[9px] font-mono leading-tight overflow-hidden">
