...
import type { GanttTask, MonthInfo, GridLine, DependencyLine, BarStyle, DragState, ZoomLevel } from './types';
    ...
    <div className="flex-1 overflow-x-auto">
      <div style={{ minWidth: `${chartWidth}%` }} ref={chartRef as React.RefObject<HTMLDivElement>}>
        ...
        <div className="relative">
            ...
            return (
              <Tooltip key={`grid-${idx}`}>
                <TooltipTrigger asChild>
                  <div
                    className={cn("absolute top-0 bottom-0 w-3 -ml-1.5 cursor-default z-[1]", "hover:bg-primary/5 transition-colors")}
                    style={{ left: `${percent}%` }}
                  >
                    <div
                      className={cn(
                        "absolute left-1/2 top-0 bottom-0 w-px",
                        isMonthStart ? "bg-border/60" : isWeekStart ? "bg-border/30" : "bg-border/15"
                      )}
                    />
                  </div>
                </TooltipTrigger>
                <TooltipContent side="top" className="text-xs">
                  ...
                  <p className="text-muted-foreground capitalize">{format(line.date, 'EEEE', { locale: ptBR })}</p>
                </TooltipContent>
              </Tooltip>
            );
          })}
          ...
          <GanttDependencyLines dependencyLines={dependencyLines} getBarStyle={getBarStyle} />
        </div>
        ...
        <div className="h-8 flex border-t border-border bg-muted/20 relative">
          ...
          {todayPercent >= 0 && todayPercent <= 100 && (
            <div className="absolute top-0 h-full flex items-center" style={{ left: `${todayPercent}%`, transform: 'translateX(-50%)' }}>
