...
import { mapCronogramaParaGantt, type CronogramaTabelaRow } from '@/lib/mapCronogramaParaGantt';
import { ganttLogger } from '@/lib/devLogger';
import { Activity } from '@/types/report';
import { safeParseLocalDate } from './utils';
import type { GanttTask, MonthInfo, GridLine, DependencyLine, BarStyle } from './types';
...
export function useGanttData(
  ...
  const { startDate, endDate, totalDays, months, gridLines } = useMemo(() => {
    ...
    if (activities.length === 0) {
      return {
        ...
        endDate: today,
        totalDays: 30,
        months: [{ date: today, label: format(today, 'MMM yyyy', { locale: ptBR }), days: 30 }] as MonthInfo[],
    ...
    const monthsList: MonthInfo[] = eachMonthOfInterval({ start, end }).map(date => ({
      date,
  ...
  const getBarStyle = useCallback((startStr: string, endStr: string): BarStyle => {
