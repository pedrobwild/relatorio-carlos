  ...
  return (
    <div
      className={cn(
        ...
        isSelected ? "bg-primary/10 ring-1 ring-inset ring-primary/30" : "hover:bg-muted/10"
      )}
    ...
    >
        ...
        <Tooltip>
          ...
          <TooltipContent>
            ...
            {(task.baselineStart !== task.plannedStart || task.baselineEnd !== task.plannedEnd) && (
              <p className="text-xs text-[hsl(var(--warning))] mt-1">⚠ Datas alteradas desde o baseline</p>
            )}
          </TooltipContent>
        </Tooltip>
      )}
          ...
          >
            {!hasActualStart && showProgressLabel && (
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-[9px] font-medium text-primary/70">Previsto</span>
              </div>
            )}
            {editable && (
              <>
                <div
                  ...
                  onMouseDown={(e) => { e.stopPropagation(); onDragStart(e, index, 'resize-start'); }}
                />
                <div
                  ...
                  onMouseDown={(e) => { e.stopPropagation(); onDragStart(e, index, 'resize-end'); }}
                />
              </>
            )}
          </div>
      ...
      {actualBarStyle && (
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              {showProgressLabel && (
                <span className="text-[10px] font-bold px-1.5 whitespace-nowrap drop-shadow-sm text-white">{progress}%</span>
              )}
            </div>
          </TooltipTrigger>
      ...
      {delayBarStyle && (
        <Tooltip>
          <TooltipTrigger asChild>
            <div className="absolute top-6 h-4 rounded-sm cursor-pointer bg-destructive" style={{ left: delayBarStyle.left, width: delayBarStyle.width }} />
          </TooltipTrigger>
          <TooltipContent side="bottom" className="text-xs">
            ...
            <p className="text-destructive font-medium">+{delayDays} {delayDays === 1 ? 'dia' : 'dias'}</p>
          </TooltipContent>
        </Tooltip>
      )}
      ...
      {remainingPlannedStyle && (
        <Tooltip>
          ...
          <TooltipContent side="bottom" className="text-xs">
            ...
            <p className="text-muted-foreground">Hoje → {format(plannedEndDate, 'dd/MM/yyyy')}</p>
          </TooltipContent>
        </Tooltip>
      )}
    </div>
