  ...
  return (
    <div className="flex items-center justify-between p-3 border-b border-border bg-muted/30">
      ...
      <div className="flex items-center gap-2">
        <Button variant="outline" size="sm" className="h-9 min-h-[44px] text-xs gap-1 px-2" onClick={onToggleFullChart}>
          {showFullChart ? (
            <>
        ...
        <Button variant="ghost" size="icon" className="h-11 w-11 min-h-[44px]" onClick={onZoomIn} disabled={!canZoomIn} aria-label="Aumentar zoom">
          <ZoomIn className="h-4 w-4" />
        </Button>
        <span className="text-xs text-muted-foreground capitalize">{zoomLevel}</span>
        <Button variant="ghost" size="icon" className="h-11 w-11 min-h-[44px]" onClick={onZoomOut} disabled={!canZoomOut} aria-label="Diminuir zoom">
