...
export function InlineRichEditor({ value, onChange, placeholder = 'Digite aqui...', minHeight = '120px', className }: InlineRichEditorProps) {
  ...
  return (
    <div className={cn('rounded-md border border-input bg-background', className)}>
      ...
      <div className="flex items-center gap-0.5 px-2 py-1 border-b border-input bg-muted/30">
        <TBtn onClick={() => exec('bold')} title="Negrito"><Bold className="w-3.5 h-3.5" /></TBtn>
        <TBtn onClick={() => exec('italic')} title="Itálico"><Italic className="w-3.5 h-3.5" /></TBtn>
        <TBtn onClick={() => exec('underline')} title="Sublinhado"><Underline className="w-3.5 h-3.5" /></TBtn>
        <div className="w-px h-4 bg-border mx-0.5" />
        <TBtn onClick={() => exec('insertUnorderedList')} title="Lista"><List className="w-3.5 h-3.5" /></TBtn>
        <TBtn onClick={() => exec('insertOrderedList')} title="Lista numerada"><ListOrdered className="w-3.5 h-3.5" /></TBtn>
      </div>
