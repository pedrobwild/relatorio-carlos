...
const statusColors: Record<string, string> = {
  draft: "bg-slate-500/10 text-slate-600 border-slate-300/50",
  active: "bg-emerald-500/10 text-emerald-700 border-emerald-300/50",
  completed: "bg-blue-500/10 text-blue-700 border-blue-300/50",
  paused: "bg-amber-500/10 text-amber-700 border-amber-300/50",
...
export function ProjectsListView({
  ...
  const [deleteTarget, setDeleteTarget] = useState<ProjectWithCustomer | null>(
    null,
  );
  ...
  const stagesMap = useMemo(() => {
    ...
    if (journeyStagesMap) {
      const entries =
        journeyStagesMap instanceof Map
  ...
  const sortedProjects = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      ...
      if (key === "obra") {
        ...
        const aDate = a.planned_end_date
          ? new Date(a.planned_end_date).getTime()
          : null;
        const bDate = b.planned_end_date
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <div className="rounded-xl border border-border/50 bg-white overflow-hidden shadow-sm">
        <Table>
          <TableHeader>
            <TableRow className="bg-white hover:bg-white border-b border-border/50">
              <TableHead className="w-7 px-1" />
              <TableHead
                className="py-2.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70 cursor-pointer hover:text-foreground transition-colors select-none"
                onClick={() => handleSort("obra")}
              >
                Obra{sortIcon("obra")}
              </TableHead>
              <TableHead
                className="w-[100px] py-2.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70 whitespace-nowrap cursor-pointer hover:text-foreground transition-colors select-none"
              ...
              <TableHead
                className="w-[56px] text-center py-2.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70 whitespace-nowrap cursor-pointer hover:text-foreground transition-colors select-none"
              ...
              <TableHead
                className="w-[96px] text-center py-2.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70 whitespace-nowrap cursor-pointer hover:text-foreground transition-colors select-none"
                onClick={() => handleSort("entrega")}
              >
                Entrega{sortIcon("entrega")}
              </TableHead>
              <TableHead
                className="w-[110px] text-center py-2.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70 whitespace-nowrap cursor-pointer hover:text-foreground transition-colors select-none"
                onClick={() => handleSort("avanco")}
              >
                Avanço{sortIcon("avanco")}
              </TableHead>
              <TableHead
                className="w-[40px] text-center py-2.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70 whitespace-nowrap cursor-pointer hover:text-foreground transition-colors select-none"
                onClick={() => handleSort("pendencias", "desc")}
              >
                ...
                >
                  <>
                    <ProjectRow
                      ...
                      onEdit={() =>
                        navigate(`/gestao/obra/${project.id}/editar`)
                      }
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja excluir a obra"{deleteTarget?.name}"? Esta
              ação é irreversível e excluirá todos os dados relacionados.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
          ...
          {project.data_recebimento_chaves && (
            <div className="flex items-center gap-1.5">
              ...
              <span>
                Chaves:{" "}
                {format(
                  parseLocalDate(project.data_recebimento_chaves),
                  "dd/MM/yyyy",
                  { locale: ptBR },
                )}
              </span>
...
function ProjectRow({
  ...
  return (
    <CollapsibleTrigger asChild>
      <TableRow
        className={cn(
          "cursor-pointer transition-colors group/row border-b border-border/30 bg-white",
          isOverdue
            ? "hover:bg-red-50/60"
            : isApproaching
      ...
      >
            ...
            <TooltipTrigger asChild>
              <Badge
                ...
                className={cn(
                  ...
                  "text-[9px] font-semibold px-1.5 py-0 h-[18px] whitespace-nowrap",
                )}
              >
        ...
        <TableCell className="text-center py-2">
          {(() => {
            const lastActivityDate = summary?.last_activity_at
              ? new Date(summary.last_activity_at)
              : null;
            const daysSinceUpdate = lastActivityDate
              ? differenceInDays(getTodayLocal(), lastActivityDate)
              : null;
            ...
            return (
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className={cn("text-[11px] tabular-nums", cls)}>
                    {label}
                  </span>
                </TooltipTrigger>
                <TooltipContent side="top" className="text-xs">
                  {format(lastActivityDate, "dd/MM/yyyy'às' HH:mm", {
                    locale: ptBR,
                  })}
                </TooltipContent>
              </Tooltip>
        ...
        <TableCell className="text-center py-2">
          {plannedEnd ? (
            <div className="flex flex-col items-center gap-0.5">
              ...
              {isFinished ? (
                <span className="text-[9px] text-emerald-600 font-medium flex items-center gap-0.5 leading-none">
                ...
                <span className="text-[9px] text-amber-600 font-medium flex items-center gap-0.5 leading-none">
        ...
        <TableCell className="text-center py-2">
          <Tooltip>
            ...
            <TooltipContent side="top" className="text-xs">
              <p>
                Real: {Math.round(progress)}% | Planejado: {plannedProgress}%
              </p>
              <p>
                Desvio: {progressDeviation >= 0 ? "+" : ""}
                {progressDeviation} p.p.
              </p>
            </TooltipContent>
          </Tooltip>
        </TableCell>
          ...
          pendingDocuments === 0 ? (
            ...
            <Tooltip>
              ...
              <TooltipContent className="text-xs space-y-0.5">
                ...
                {pendingCount - overdueCount > 0 && (
                  <p>{pendingCount - overdueCount} pendente(s) no prazo</p>
                )}
                {unsignedFormalizations > 0 && (
                  <p>{unsignedFormalizations} formalização(ões) p/ assinar</p>
                )}
                {pendingDocuments > 0 && (
                  <p>{pendingDocuments} documento(s) pendente(s)</p>
                )}
              </TooltipContent>
            </Tooltip>
          )}
          ...
          <DropdownMenu>
            ...
            >
              ...
              {project.status === "draft" && (
                <DropdownMenuItem
                  onClick={() => navigate(`/gestao/obra/${project.id}/wizard`)}
                >
