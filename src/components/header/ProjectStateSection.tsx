...
import { CalendarIcon, CheckCircle2, ChevronRight, AlertCircle, Pencil, X } from "lucide-react";
...
import { formatDateShort, formatDateFull, type ProjectMetrics, type MilestoneItem, type MilestoneKey } from "./types";
...
export function ProjectStateSection({
  metrics, displayStartDate, displayEndDate, endDate, dateChangeInfo, onShowDateChangeAlert,
}: ProjectStateSectionProps) {
  return (
    <div className="px-6 py-3 border-t border-border bg-muted/30">
      <div className="flex items-start gap-6 flex-wrap">
        <div className="flex-1 min-w-0">
          <span className="text-meta font-semibold uppercase tracking-wider block mb-1">Etapa Atual</span>
          <p className="text-body font-semibold text-foreground break-words leading-snug">{metrics.currentActivity}</p>
          <Badge variant="secondary" className="mt-1.5 text-meta px-2 py-0.5 h-auto font-medium tabular-nums">
            {metrics.completedActivities} de {metrics.totalActivities} atividades
          </Badge>
        </div>
        ...
        <div className="flex items-center gap-3 shrink-0 self-center">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              onClick={endDate === dateChangeInfo.originalDate ? onShowDateChangeAlert : undefined}
              className={cn(
            ...
            {endDate === dateChangeInfo.originalDate && <AlertCircle className="w-3 h-3 text-warning" />}
          </div>
        </div>
      </div>
    </div>
...
export function MilestonesBar({ milestoneItems, canEdit, isMobile = false, onMilestoneDateChange }: MilestonesBarProps) {
  ...
  const handleSelect = useCallback(async (key: MilestoneKey, date: Date | undefined) => {
    ...
    try {
      await onMilestoneDateChange(key, date ? date.toISOString().split('T')[0] : null);
    } finally {
  ...
  const handleClear = useCallback(async (key: MilestoneKey) => {
    if (!onMilestoneDateChange) return;
    setSavingMilestone(true);
    try {
                ...
                <Popover open={editingMilestone === m.key} onOpenChange={(open) => setEditingMilestone(open ? m.key : null)}>
                  <PopoverTrigger asChild>
                    <button className="flex items-baseline justify-between gap-1 w-full rounded px-1 -mx-1 hover:bg-accent/60 transition-colors">
                      ...
                      <span className={cn("text-xs font-semibold tabular-nums shrink-0", m.value ? "text-foreground" : "text-muted-foreground/40")}>
                        {m.value ? formatDateShort(m.value) : "—"}
                      </span>
                    </button>
                  </PopoverTrigger>
                  <MilestonePopoverContent milestone={m} savingMilestone={savingMilestone} onSelect={handleSelect} onClear={handleClear} onClose={() => setEditingMilestone(null)} />
                </Popover>
              ) : (
                <>
                  <span className="text-caption truncate">{m.label}</span>
          ...
          {visibleItems.map((m, i) => (
            <div key={m.label} className="flex items-center gap-3">
              ...
              {canEdit && onMilestoneDateChange ? (
                <Popover open={editingMilestone === m.key} onOpenChange={(open) => setEditingMilestone(open ? m.key : null)}>
                  <PopoverTrigger asChild>
                    <button className={cn("flex items-baseline gap-1.5 rounded-md px-1.5 py-0.5 -mx-1.5 -my-0.5 transition-colors hover:bg-accent/60 cursor-pointer group")}>
                      <span className="text-caption whitespace-nowrap">{m.label}</span>
                      <span className={cn("text-xs font-semibold tabular-nums whitespace-nowrap", m.value ? "text-foreground" : "text-muted-foreground/50")}>
...
function MilestonePopoverContent({
  milestone, savingMilestone, onSelect, onClear, onClose,
}: {
    ...
    <PopoverContent className="w-auto p-0" align="start" onPointerDownOutside={(e) => e.preventDefault()} onInteractOutside={(e) => e.preventDefault()}>
      <div className="p-2 border-b border-border flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          {milestone.value && (
            <Button variant="ghost" size="sm" className="h-6 px-2 text-xs text-destructive hover:text-destructive" onClick={() => onClear(milestone.key)} disabled={savingMilestone}>
              <X className="w-3 h-3 mr-1" />Limpar
            </Button>
          )}
          <Button variant="ghost" size="sm" className="h-6 px-2 text-xs" onClick={onClose}>Fechar</Button>
        </div>
      </div>
      <Calendar
        mode="single"
        selected={milestone.value ? new Date(milestone.value + "T00:00:00") : undefined}
