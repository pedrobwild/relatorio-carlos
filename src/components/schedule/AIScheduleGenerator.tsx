...
import { Wand2, Upload, FileSpreadsheet, Loader2, CalendarDays, ShoppingCart, AlertTriangle, ChevronDown, ChevronRight, Lightbulb } from 'lucide-react';
  ...
  const parseFile = useCallback((file: File) => {
    ...
    if (ext === 'csv') {
      Papa.parse(file, {
        ...
        skipEmptyLines: true,
        complete: (results) => {
          const items = mapRowsToItems(results.data as Record<string, string>[]);
    ...
    return rows
      .map((row) => {
        ...
        const descKey = keys.find((k) =>
          /descri|item|servi|atividade|nome/i.test(k)
        ) || keys[0];
        const qtyKey = keys.find((k) => /qtd|quant/i.test(k));
        const unitKey = keys.find((k) => /und|unid|un\b/i.test(k));
        const valKey = keys.find((k) => /valor|custo|preco|total|preço/i.test(k));
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              Importe o orçamento da obra e a IA vai sugerir o melhor cronograma semanal e lista de compras.
            </DialogDescription>
          </DialogHeader>
            ...
            {!plan ? (
              <div className="space-y-5 py-2">
                ...
                {budgetItems.length > 0 && (
                  <div className="space-y-2">
                    <Label className="text-sm font-medium">Itens encontrados:</Label>
                      ...
                      <div className="p-2 space-y-1">
                        {budgetItems.slice(0, 20).map((item, i) => (
                          <div key={i} className="flex items-center justify-between text-xs py-1 px-2 rounded hover:bg-muted/50">
                            ...
                            {item.value && <span className="text-muted-foreground ml-2 shrink-0">R$ {item.value}</span>}
                          </div>
                        ))}
              ...
              <div className="flex flex-col h-full overflow-hidden">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 py-3">
                  ...
                  <Card className="p-3">
                    <div className="text-2xl font-bold text-primary">{plan.summary.totalActivities}</div>
                ...
                <div className="flex gap-1 bg-muted rounded-lg p-1 mb-3">
                  <button
                    className={cn('flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors', activeTab === 'schedule' ? 'bg-background shadow-sm' : 'text-muted-foreground hover:text-foreground')}
                    onClick={() => setActiveTab('schedule')}
                  >
                    <CalendarDays className="h-3.5 w-3.5" /> Cronograma
                  </button>
                  <button
                    className={cn('flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors', activeTab === 'purchases' ? 'bg-background shadow-sm' : 'text-muted-foreground hover:text-foreground')}
                        ...
                        <div key={i} className="flex items-start gap-3 p-3 rounded-lg border bg-card text-sm">
                          <div className="flex-1 min-w-0 space-y-1">
                            <div className="flex items-center gap-2">
                              ...
                              <Badge variant="outline" className={cn('text-[10px] h-5', priorityColors[purchase.priority])}>
                                {purchase.priority}
                              </Badge>
                            </div>
                            <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-muted-foreground">
                              <span>{purchase.category}</span>
                              {purchase.quantity && <span>Qtd: {purchase.quantity}</span>}
                              {purchase.estimatedCost && <span>~{purchase.estimatedCost}</span>}
                            </div>
                            <div className="flex gap-3 text-xs">
                              <span className="text-amber-600">Pedir: Sem. {purchase.orderByWeek}</span>
                              <span className="text-primary">Usar: Sem. {purchase.neededByWeek}</span>
                              <span className="text-muted-foreground">{purchase.leadTimeDays}d entrega</span>
                            </div>
                ...
                {/* Recommendations */}
                {plan.summary.recommendations && plan.summary.recommendations.length > 0 && (
                  <div className="mt-3 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                    ...
                    <ul className="space-y-0.5">
                      {plan.summary.recommendations.map((rec, i) => (
                        <li key={i} className="text-xs text-amber-700/80">• {rec}</li>
                      ))}
                    </ul>
                  </div>
                )}
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            {plan ? (
              <>
                <Button variant="outline" onClick={handleReset}>Novo Orçamento</Button>
    ...
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger asChild>
        <button className="w-full flex items-center gap-2 p-3 rounded-lg border bg-card hover:bg-accent/50 transition-colors text-left">
          ...
          <Badge className="bg-primary/15 text-primary border-primary/30 text-xs">Sem. {week.week}</Badge>
          <span className="text-sm font-medium">{week.phase}</span>
          <span className="text-xs text-muted-foreground ml-auto">{week.activities.length} atividades</span>
        </button>
      </CollapsibleTrigger>
      <CollapsibleContent>
        <div className="ml-6 mt-1 space-y-1">
          {week.activities.map((act, i) => (
            <div key={i} className="flex items-start gap-2 p-2 rounded-md border-l-2 border-primary/30 bg-muted/30 text-sm">
              <div className="flex-1 space-y-0.5">
                ...
                {act.notes && <p className="text-[11px] text-muted-foreground italic">{act.notes}</p>}
              </div>
            </div>
          ))}
        </div>
