...
export function InspectionsList({
  ...
  const [filterStatus, setFilterStatus] = useState<InspectionStatus | null>(
    null,
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (filterTypes.length > 0)
      result = result.filter((i) =>
        filterTypes.includes((i.inspection_type || "rotina") as InspectionType),
      );
    if (searchQuery.trim()) {
      result = result.filter((i) =>
  ...
  const usedTypes = useMemo(() => {
    const types = new Set(
      inspections.map((i) => (i.inspection_type || "rotina") as InspectionType),
    );
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-3">
          {filtered.map((inspection) => {
            const cfg =
              statusConfig[inspection.status as InspectionStatus] ||
              statusConfig.draft;
                  ...
                  <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-medium text-sm sm:text-base">
                            Vistoria{" "}
                            {format(
                              parseISO(inspection.inspection_date),
                              "dd/MM/yyyy",
                              { locale: ptBR },
                            )}
                          </span>
                          ...
                          {(ncCountByInspection[inspection.id] ?? 0) > 0 && (
                            ...
                            >
                              ...
                              {ncCountByInspection[inspection.id]} NC
                              {ncCountByInspection[inspection.id] > 1
                                ? "s"
                                : ""}
                            </Badge>
                          )}
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <div className="hidden sm:flex items-center gap-1 text-xs text-muted-foreground">
                        ...
                        {format(parseISO(inspection.created_at), "dd/MM", {
                          locale: ptBR,
                        })}
                      </div>
