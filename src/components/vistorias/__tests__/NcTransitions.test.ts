...
type NcStatus = 'open' | 'in_treatment' | 'pending_verification' | 'pending_approval' | 'closed' | 'reopened';
...
async function executeTransition(
  ...
  deps: {
    saveEvidence: (p: { id: string; project_id: string; evidence_photos_before: string[]; evidence_photos_after: string[] }) => Promise<void>;
    updateNc: (p: { id: string; project_id: string; root_cause?: string; actual_cost?: number | null }) => Promise<void>;
    transitionStatus: (p: { newStatus: NcStatus; notes?: string; corrective_action?: string; resolution_notes?: string; evidence_photos_before?: string[]; evidence_photos_after?: string[] }) => Promise<void>;
  },
) {
  ...
  if (params.newStatus === 'closed') {
    await deps.updateNc({
      ...
      actual_cost: params.actualCostInput ? parseFloat(params.actualCostInput) : null,
    });
  }
  ...
  await deps.transitionStatus({
    ...
    corrective_action: params.newStatus === 'in_treatment' ? params.correctiveAction : undefined,
    resolution_notes: params.newStatus === 'pending_verification' ? params.actionNotes : undefined,
    evidence_photos_before: params.photosBefore.length > 0 ? params.photosBefore : undefined,
    evidence_photos_after: params.photosAfter.length > 0 ? params.photosAfter : undefined,
  });
}

describe('NC Transition Orchestration', () => {
  const callOrder: string[] = [];
  const saveEvidence = vi.fn(async () => { callOrder.push('saveEvidence'); });
  const updateNc = vi.fn(async () => { callOrder.push('updateNc'); });
  const transitionStatus = vi.fn(async () => { callOrder.push('transitionStatus'); });
  ...
  it('saves evidence → updateNc → transition when closing', async () => {
    ...
    expect(updateNc).toHaveBeenCalledWith(
      expect.objectContaining({ root_cause: 'Falha de material', actual_cost: 1500 }),
    );
    expect(transitionStatus).toHaveBeenCalledWith(
      expect.objectContaining({
        newStatus: 'closed',
