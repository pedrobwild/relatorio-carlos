...
const severityOrder: Record<NcSeverity, number> = {
  critical: 0,
  high: 1,
  medium: 2,
  low: 3,
...
export function NonConformitiesList({
  ...
  const filtered = useMemo(() => {
    ...
    if (summaryFilter) {
      switch (summaryFilter.type) {
        ...
        case "severity":
          result = result.filter(
            (nc) =>
              summaryFilter.value.includes(nc.severity) &&
              nc.status !== "closed",
          );
          break;
        case "status":
        ...
        case "category":
          result = result.filter(
            (nc) =>
              nc.category === summaryFilter.value && nc.status !== "closed",
          );
          break;
      }
    }
    ...
    if (filterStatus)
      result = result.filter((nc) => nc.status === filterStatus);
    if (filterSeverity)
      result = result.filter((nc) => nc.severity === filterSeverity);
    if (filterCategories.length > 0)
      result = result.filter((nc) =>
        filterCategories.includes(nc.category || ""),
      );
    if (filterOverdue)
      result = result.filter(
            ...
            <div className="flex flex-wrap gap-1.5">
              {allSeverities.map((s) => (
                <Button
                  ...
                  className="h-8 text-xs min-w-[44px]"
                  onClick={() =>
                    setFilterSeverity((prev) => (prev === s ? null : s))
                  }
                >
            ...
            <div className="flex flex-wrap gap-1.5">
              {NC_CATEGORIES.map((cat) => (
                <Button
                  key={cat}
                  variant={
                    filterCategories.includes(cat) ? "default" : "outline"
                  }
                  size="sm"
                  className="h-8 text-xs min-w-[44px]"
            ...
            return (
              <Card
                ...
                className={`cursor-pointer hover:border-primary/50 transition-colors active:scale-[0.98] ${
                  isExpiringSoon
                    ? "ring-2 ring-orange-400/60 animate-pulse"
                    : ""
                }`}
                onClick={() => onSelect(nc)}
              >
                    ...
                    <div className="flex items-start sm:items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                          <span
                            className={`text-[10px] sm:text-xs font-semibold px-1.5 py-0.5 rounded ${sev.className}`}
                          >
                            {sev.label}
                          </span>
                          <Badge
                          ...
                          {isExpiringSoon && !isOverdue && (
                            <Badge
                              variant="outline"
                              className="gap-1 text-[10px] sm:text-xs border-orange-400 text-orange-600"
                            >
                              <Clock className="h-3 w-3" />
                          ...
                          {reopenCount > 0 && (
                            <Badge
                              variant={
                                reopenCount >= 3 ? "destructive" : "outline"
                              }
                              className="gap-1 text-[10px] sm:text-xs"
                            >
                    ...
                    <div className="flex items-center gap-1.5 shrink-0 mt-1 sm:mt-0">
                      {nc.deadline && (
                        <span className="text-[10px] sm:text-xs text-muted-foreground hidden sm:inline">
                          {format(parseISO(nc.deadline), "dd/MM", {
                            locale: ptBR,
                          })}
                        </span>
                      )}
