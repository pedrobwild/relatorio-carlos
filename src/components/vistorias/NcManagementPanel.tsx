...
const statusConfig: Record<NcStatus, { label: string; shortLabel: string; variant: 'default' | 'secondary' | 'destructive' | 'outline' }> = {
  open: { label: 'Abertas', shortLabel: 'Abertas', variant: 'destructive' },
  in_treatment: { label: 'Em tratamento', shortLabel: 'Tratamento', variant: 'default' },
  pending_verification: { label: 'Verificação', shortLabel: 'Verificação', variant: 'outline' },
  pending_approval: { label: 'Aprovação', shortLabel: 'Aprovação', variant: 'outline' },
  reopened: { label: 'Reaberta', shortLabel: 'Reaberta', variant: 'destructive' },
  closed: { label: 'Encerradas', shortLabel: 'Encerradas', variant: 'secondary' },
};
...
export function NcManagementPanel({ nonConformities, searchQuery, onSelect, onCreateNc, canCreate, showProjectBadge }: Props) {
  ...
  const metrics = useMemo(() => {
    ...
    const pendingAction = nonConformities.filter(nc => 
      nc.status === 'pending_verification' || nc.status === 'pending_approval'
    );
  ...
  const filteredNcs = useMemo(() => {
    ...
    return [...result].sort((a, b) => {
      const aOverdue = a.deadline && a.deadline < today && a.status !== 'closed' ? 1 : 0;
      const bOverdue = b.deadline && b.deadline < today && b.status !== 'closed' ? 1 : 0;
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
        ...
        <div className="ml-auto flex items-center gap-1.5 shrink-0">
          ...
          {canCreate && (
            <Button onClick={onCreateNc} size="sm" className="gap-1.5 h-9 shrink-0">
              <Plus className="h-4 w-4" />
      ...
      {filteredNcs.length === 0 ? (
        <EmptyState
          icon={AlertTriangle}
          title={viewTab === 'closed' ? 'Nenhuma NC encerrada' : 'Nenhuma NC pendente'}
          description={effectiveSearch ? 'Nenhuma NC corresponde à busca.' : 'Todas as NCs foram resolvidas! 🎉'}
        />
      ) : (
      ...
      <Collapsible open={showAnalytics} onOpenChange={setShowAnalytics}>
        <CollapsibleTrigger asChild>
          <Button variant="ghost" className="w-full gap-2 h-10 text-sm text-muted-foreground justify-center">
...
function UrgentChip({ icon, label, danger, warning }: { 
  icon: React.ReactNode; label: string; danger?: boolean; warning?: boolean 
}) {
  return (
    <div className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs font-medium whitespace-nowrap shrink-0 ${
      danger 
...
function TabButton({ active, onClick, children, count, danger }: {
  active: boolean; onClick: () => void; children: React.ReactNode; count: number; danger?: boolean;
}) {
  return (
    <button
      onClick={onClick}
        ...
        <div className="flex items-start gap-2.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
              <Badge variant={st.variant} className="text-[10px] h-5">{st.shortLabel}</Badge>
