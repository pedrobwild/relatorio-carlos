...
const severityConfig: Record<NcSeverity, { label: string; className: string; order: number }> = {
  critical: { label: 'Crítica', className: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', order: 0 },
  high: { label: 'Alta', className: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400', order: 1 },
  medium: { label: 'Média', className: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400', order: 2 },
  low: { label: 'Baixa', className: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400', order: 3 },
};
...
const COLUMNS: KanbanColumn[] = [
  { status: 'open', label: 'Abertas', dotColor: 'bg-red-500', borderColor: 'border-t-red-500', bgColor: 'bg-red-50/60 dark:bg-red-950/20' },
  { status: 'in_treatment', label: 'Em Tratamento', dotColor: 'bg-blue-500', borderColor: 'border-t-blue-500', bgColor: 'bg-blue-50/60 dark:bg-blue-950/20' },
  { status: 'pending_verification', label: 'Verificação', dotColor: 'bg-amber-500', borderColor: 'border-t-amber-500', bgColor: 'bg-amber-50/60 dark:bg-amber-950/20' },
  { status: 'pending_approval', label: 'Aprovação', dotColor: 'bg-purple-500', borderColor: 'border-t-purple-500', bgColor: 'bg-purple-50/60 dark:bg-purple-950/20' },
  { status: 'closed', label: 'Encerradas', dotColor: 'bg-green-500', borderColor: 'border-t-green-500', bgColor: 'bg-green-50/60 dark:bg-green-950/20' },
];
...
export function NcKanbanView({ nonConformities, searchQuery, onSelect, showProjectBadge }: Props) {
  ...
  const getColumnNcs = (status: NcStatus) => {
    const ncs = status === 'open'
      ? filtered.filter(nc => nc.status === 'open' || nc.status === 'reopened')
...
function NcKanbanCard({ nc, today, onSelect, showProjectBadge }: { nc: NonConformity & { project_name?: string | null }; today: string; onSelect: (nc: NonConformity) => void; showProjectBadge?: boolean }) {
  ...
  const daysLeft = deadlineDate && nc.status !== 'closed'
  ...
  if (nc.deadline) {
    ...
    if (nc.status === 'closed') {
      deadlineDisplay = <span className="text-muted-foreground">{formatted}</span>;
    } else if (isOverdue) {
      const daysOverdue = Math.abs(differenceInDays(new Date(), deadlineDate!));
      deadlineDisplay = <span className="text-destructive font-semibold">{daysOverdue}d atraso · {formatted}</span>;
    } else if (isExpiringSoon) {
      deadlineDisplay = <span className="text-orange-600 dark:text-orange-400 font-semibold">{daysLeft === 0 ? 'Hoje' : `${daysLeft}d`} · {formatted}</span>;
    } else {
      deadlineDisplay = <span className="text-muted-foreground">{formatted}</span>;
    }
  }
      ...
      <CardContent className="p-2.5 space-y-1.5">
        ...
        <div className="flex items-start gap-2">
          <div className={cn(
            ...
            nc.severity === 'high' ? 'bg-orange-500' :
            nc.severity === 'medium' ? 'bg-yellow-500' : 'bg-blue-400'
          )} />
          <span className="font-semibold text-sm leading-tight line-clamp-2 flex-1">{nc.title}</span>
        </div>

        {nc.description && (
          <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed">{nc.description}</p>
        )}

        <div className="flex flex-wrap items-center gap-1.5">
          <span className={cn('text-[10px] font-semibold px-1.5 py-0.5 rounded', sev.className)}>
            {sev.label}
          </span>
          {nc.category && (
            <span className="text-[10px] text-muted-foreground flex items-center gap-0.5">
              <Tag className="h-2.5 w-2.5" />{nc.category}
            </span>
          )}
          {nc.status === 'reopened' && (
            <Badge variant="destructive" className="text-[10px] h-5 gap-0.5">
              <RotateCcw className="h-2.5 w-2.5" />Reaberta
            </Badge>
          )}
          {reopenCount > 1 && (
            <Badge variant="outline" className="text-[10px] h-5 gap-0.5">
              <RotateCcw className="h-2.5 w-2.5" />{reopenCount}x
            </Badge>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
          {nc.responsible_user_name && (
            <span className="flex items-center gap-1 bg-muted/50 rounded-md px-1.5 py-0.5 truncate max-w-[140px]">
              <User className="h-3 w-3 shrink-0" />{nc.responsible_user_name}
            </span>
          )}
          {deadlineDisplay && (
            <span className="flex items-center gap-1 rounded-md px-1.5 py-0.5 bg-muted/50">
              <Clock className="h-3 w-3" />{deadlineDisplay}
            </span>
          )}
        </div>
      </CardContent>
