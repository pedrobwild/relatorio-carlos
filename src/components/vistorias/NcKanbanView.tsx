...
const severityConfig: Record<NcSeverity, { label: string; className: string; order: number }> = {
 critical: { label:'Crítica', className:'bg-red-100 text-red-700', order: 0 },
 high: { label:'Alta', className:'bg-orange-100 text-orange-700', order: 1 },
 medium: { label:'Média', className:'bg-yellow-100 text-yellow-700', order: 2 },
...
const COLUMNS: KanbanColumn[] = [
 { status:'open', label:'Abertas', dotColor:'bg-red-500', borderColor:'border-t-red-500', bgColor:'bg-red-50/60' },
 { status:'in_treatment', label:'Em Tratamento', dotColor:'bg-blue-500', borderColor:'border-t-blue-500', bgColor:'bg-blue-50/60' },
 { status:'pending_verification', label:'Verificação', dotColor:'bg-amber-500', borderColor:'border-t-amber-500', bgColor:'bg-amber-50/60' },
 { status:'pending_approval', label:'Aprovação', dotColor:'bg-purple-500', borderColor:'border-t-purple-500', bgColor:'bg-purple-50/60' },
 { status:'closed', label:'Encerradas', dotColor:'bg-green-500', borderColor:'border-t-green-500', bgColor:'bg-green-50/60' },
];
...
function NcKanbanCard({ nc, today, onSelect, showProjectBadge }: { nc: NonConformity & { project_name?: string | null }; today: string; onSelect: (nc: NonConformity) => void; showProjectBadge?: boolean }) {
 ...
 deadlineDisplay = <span className="text-orange-600 font-semibold">{daysLeft === 0 ?'Hoje' :`${daysLeft}d`} · {formatted}</span>;
 } else {
 deadlineDisplay = <span className="text-muted-foreground">{formatted}</span>;
 }
 }
