...
function TimelineContent({ events, isLoading, error }: { events: DomainEvent[]; isLoading: boolean; error: Error | null }) {
  ...
  if (events.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-xs text-muted-foreground/70 mt-1">As ações do projeto aparecerão aqui.</p>
      </div>
    );
  }
...
export function ActivityTimeline({ projectId, orgId, maxItems = 20, showHeader = true, className = '' }: ActivityTimelineProps) {
  ...
  if (showHeader) {
    return (
      <Card className={className}>
        ...
        <CardContent>
          <TimelineContent events={displayEvents} isLoading={isLoading} error={error} />
        </CardContent>
      </Card>
    );
  }
...
export function ActivityTimelineCompact({ projectId, maxItems = 5 }: { projectId?: string; maxItems?: number }) {
  ...
  if (displayEvents.length === 0) {
    return <p className="text-xs text-muted-foreground text-center py-4">Sem atividade recente</p>;
  }
      ...
      {displayEvents.map((event) => {
        ...
        return (
          <div key={event.id} className="flex items-center gap-2 text-xs">
            ...
            <span className="text-muted-foreground shrink-0">{formatRelativeDate(event.created_at)}</span>
          </div>
        );
      })}
    </div>
