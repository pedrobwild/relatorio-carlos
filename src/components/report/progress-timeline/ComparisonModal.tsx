...
import { ArrowLeftRight, Box, Camera, X, Maximize2, Minimize2, ZoomIn, ZoomOut, RotateCcw } from 'lucide-react';
...
export function ComparisonModal({ isOpen, onClose, render3D, realPhoto, roomName }: ComparisonModalProps) {
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    if (e.touches.length === 2) {
      ...
      lastTouchRef.current = { x: (t1.clientX + t2.clientX) / 2, y: (t1.clientY + t2.clientY) / 2, distance };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={cn('p-0 bg-card border-border overflow-hidden', isFullscreen ? 'max-w-full h-full' : 'max-w-4xl')}>
        <VisuallyHidden>
        ...
        <div
          ref={containerRef}
          className={cn('relative overflow-hidden touch-none', isFullscreen ? 'fixed inset-0 z-50 bg-black' : 'aspect-video')}
