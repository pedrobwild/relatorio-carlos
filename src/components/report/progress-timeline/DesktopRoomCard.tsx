...
function DesktopPhaseGrid({ room, onCompare }: { room: RoomProgress; onCompare: () => void }) {
  ...
  return (
    <div className={cn('grid gap-2', room.render3D ? 'grid-cols-4' : 'grid-cols-3')}>
      {phases.map((phase) => {
        ...
        if (!photo) {
          return (
            <div key={phase} className="aspect-[4/3] rounded-lg border-2 border-dashed border-muted-foreground/20 bg-muted/30 flex flex-col items-center justify-center gap-1">
        ...
        return (
          <button key={phase} onClick={isClickable ? onCompare : undefined} className={cn('relative aspect-[4/3] rounded-lg overflow-hidden group', isClickable && 'cursor-pointer')}>
            ...
            <div className={cn('absolute top-1.5 left-1.5 px-1.5 py-0.5 rounded-full text-[10px] font-medium flex items-center gap-1', config.color)}>
