...
const LookaheadSection = ({ tasks, onAdd, onUpdate, onRemove }: LookaheadSectionProps) => (
          ...
          <CardContent className="p-3 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input type="date" value={task.date} onChange={(e) => onUpdate(index, "date", e.target.value)} />
              </div>
              <div>
                <Label className="text-xs">Responsável</Label>
                <Input placeholder="Nome do responsável" value={task.responsible} onChange={(e) => onUpdate(index, "responsible", e.target.value)} />
              </div>
            </div>
            <div>
              <Label className="text-xs">Descrição</Label>
              <Input placeholder="Descreva a tarefa" value={task.description} onChange={(e) => onUpdate(index, "description", e.target.value)} />
            </div>
            <div>
              <Label className="text-xs">Pré-requisitos</Label>
              <Input placeholder="O que precisa estar pronto antes" value={task.prerequisites} onChange={(e) => onUpdate(index, "prerequisites", e.target.value)} />
            </div>
            <div>
              <Label className="text-xs">Risco</Label>
              <Select value={task.risk} onValueChange={(v) => onUpdate(index, "risk", v)}>
