...
const DecisionsSection = ({ decisions, onAdd, onUpdate, onRemove }: DecisionsSectionProps) => (
  <AccordionItem value="decisions" className="bg-card border border-border rounded-lg overflow-hidden">
    <AccordionTrigger className="px-4 py-3 hover:no-underline hover:bg-muted/50">
      <div className="flex items-center gap-2">
        ...
        <span className="font-semibold">Decisões do Cliente ({decisions.length})</span>
      </div>
    </AccordionTrigger>
        ...
        <Card key={decision.id} className="border-muted">
          <CardContent className="p-3 space-y-3">
            ...
            <div>
              ...
              <Textarea placeholder="Descreva a decisão necessária" value={decision.description} onChange={(e) => onUpdate(index, "description", e.target.value)} className="min-h-[60px]" />
            </div>
            <div>
              <Label className="text-xs">Impacto se Atrasado</Label>
              <Input placeholder="O que acontece se não for decidido a tempo" value={decision.impactIfDelayed} onChange={(e) => onUpdate(index, "impactIfDelayed", e.target.value)} />
            </div>
            <div>
              <Label className="text-xs">Prazo</Label>
              <Input type="date" value={decision.dueDate} onChange={(e) => onUpdate(index, "dueDate", e.target.value)} />
            </div>
          </CardContent>
        </Card>
      ))}
