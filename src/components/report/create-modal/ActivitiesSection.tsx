...
export const ActivitiesSection = ({ activities, setActivities }: ActivitiesSectionProps) => {
      ...
      <div className="space-y-4">
        {activities.map((activity, index) => {
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-tiny font-medium">Atividade #{index + 1}</span>
                {activities.length > 1 && (
                  <Button
                    type="button"
                    variant="ghost"
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                <div className="space-y-1">
                  ...
                  <Input placeholder="DD/MM" value={activity.plannedStart} onChange={(e) => updateActivity(index, 'plannedStart', e.target.value)} className="text-sm" />
                </div>
                <div className="space-y-1">
                  <Label className="text-tiny">Fim Prev.</Label>
                  <Input placeholder="DD/MM" value={activity.plannedEnd} onChange={(e) => updateActivity(index, 'plannedEnd', e.target.value)} className="text-sm" />
                </div>
                <div className="space-y-1">
                  <Label className="text-tiny">Início Real</Label>
                  <Input placeholder="DD/MM" value={activity.actualStart} onChange={(e) => updateActivity(index, 'actualStart', e.target.value)} className="text-sm" />
                </div>
                <div className="space-y-1">
                  <Label className="text-tiny">Fim Real</Label>
                  <Input placeholder="DD/MM" value={activity.actualEnd} onChange={(e) => updateActivity(index, 'actualEnd', e.target.value)} className="text-sm" />
                </div>
              </div>

              {/* Collapsible detailed description */}
              {hasDetail ? (
                <Collapsible open={isDetailOpen} onOpenChange={() => toggleDetail(index)}>
                  ...
                  <CollapsibleContent className="pt-1">
                    <Textarea
                      ...
                      onChange={(e) => updateActivity(index, 'detailed_description' as any, e.target.value)}
