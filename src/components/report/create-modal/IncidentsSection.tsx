...
import { Plus, Trash2, AlertOctagon, ChevronDown, ChevronUp } from 'lucide-react';
...
export const IncidentsSection = ({ incidents, setIncidents }: IncidentsSectionProps) => {
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input type="date" value={incident.occurrenceDate} onChange={(e) => updateIncident(index, 'occurrenceDate', e.target.value)} className="text-sm" />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs text-muted-foreground">Previsão de Resolução</Label>
                    <Input type="date" value={incident.expectedResolutionDate} onChange={(e) => updateIncident(index, 'expectedResolutionDate', e.target.value)} className="text-sm" />
                  </div>
                </div>

                {(['occurrence', 'cause', 'action', 'impact'] as const).map((field) => {
                  const labels: Record<string, { label: string; placeholder: string }> = {
                    occurrence: { label: 'Ocorrência', placeholder: 'Descreva o que aconteceu...' },
                    cause: { label: 'Causa', placeholder: 'Qual foi a causa do problema?' },
                    action: { label: 'Ação', placeholder: 'Quais ações foram tomadas?' },
                    impact: { label: 'Impacto', placeholder: 'Qual o impacto no projeto?' },
                  };
                  return (
                    <div key={field} className="space-y-1">
                      <Label className="text-xs text-muted-foreground">{labels[field].label}</Label>
                      <Textarea
                        placeholder={labels[field].placeholder}
                        value={incident[field]}
                        onChange={(e) => updateIncident(index, field, e.target.value)}
