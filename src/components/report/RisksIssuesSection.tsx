...
import { AlertTriangle, ChevronDown, CheckCircle2, MessageCircleQuestion } from "lucide-react";
...
const RisksIssuesSection = ({ issues, isStaff, onResolve, onClarify }: RisksIssuesSectionProps) => {
    ...
    <div className="bg-card rounded-lg border border-border overflow-hidden">
      ...
      <div className="hidden sm:block">
        <div className="divide-y divide-border">
          {visibleDesktop.map((issue) => (
            <RiskItem key={issue.id} issue={issue} isStaff={isStaff} onResolve={onResolve} onClarify={onClarify} />
          ))}
        </div>
          ...
          <Collapsible open={showAllDesktop} onOpenChange={setShowAllDesktop}>
            <CollapsibleContent className="divide-y divide-border overflow-hidden">
              {hiddenDesktop.map((issue, index) => (
                <RiskItem key={issue.id} issue={issue} isStaff={isStaff} onResolve={onResolve} onClarify={onClarify} animationDelay={showAllDesktop ? (index + 1) * 50 : 0} />
              ))}
            </CollapsibleContent>
            <CollapsibleTrigger asChild>
              <button className="w-full py-2 px-3 border-t border-border flex items-center justify-center gap-1.5 text-tiny font-medium text-primary hover:bg-primary/5 transition-colors">
                <span>{showAllDesktop ? "Ver menos" : `Ver todos ${issues.length} itens`}</span>
                {!showAllDesktop && <span className="bg-primary/10 px-1.5 py-0.5 rounded-md text-tiny font-semibold">+{hiddenDesktop.length}</span>}
                <ChevronDown className={cn("w-3.5 h-3.5 transition-transform duration-200", showAllDesktop && "rotate-180")} />
              </button>
            </CollapsibleTrigger>
          </Collapsible>
        )}
      ...
      <div className="sm:hidden">
        <Collapsible open={isOpen} onOpenChange={setIsOpen}>
          <div className="divide-y divide-border">
            {firstIssue && <RiskItem issue={firstIssue} isStaff={isStaff} onResolve={onResolve} onClarify={onClarify} />}
            
            <CollapsibleContent className="divide-y divide-border overflow-hidden">
              {remainingIssues.map((issue, index) => (
                <RiskItem key={issue.id} issue={issue} isStaff={isStaff} onResolve={onResolve} onClarify={onClarify} animationDelay={isOpen ? (index + 1) * 50 : 0} />
              ))}
            </CollapsibleContent>
          </div>
