  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2">
        {(isOverdue || isUrgent) && (
          <AlertCircle className={cn("w-4 h-4 shrink-0 mt-0.5", isOverdue ? "text-destructive" : "text-[hsl(var(--warning))]")} />
        )}
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground mt-0.5 leading-[1.5]">
            até <span className="font-semibold text-foreground">{format(dueDate, "EEEE (dd/MM)", { locale: ptBR })}</span>
            {decision.impactIfDelayed && (
              <> para <span className="text-foreground/80">{decision.impactIfDelayed.toLowerCase()}</span></>
            )}
          </p>
        </div>
        <Badge 
      ...
      <div className="flex items-center gap-2 ml-6">
        <div className="flex items-center gap-1">
          ...
          <span className={cn(
            ...
            isOverdue ? "text-destructive" : isUrgent ? "text-[hsl(var(--warning))]" : "text-muted-foreground"
          )}>
            {isOverdue 
