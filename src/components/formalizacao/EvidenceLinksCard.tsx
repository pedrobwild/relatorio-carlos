...
export function EvidenceLinksCard({ formalizationId, evidenceLinks }: EvidenceLinksCardProps) {
  ...
  const handleAddLink = async () => {
    if (!newLink.url) {
      toast({ title: 'URL obrigatória', description: 'Informe a URL do link.', variant: 'destructive' });
      return;
    }
    try {
        ...
        <Dialog open={linkDialogOpen} onOpenChange={setLinkDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Adicione um link externo como evidência (gravação, documento, etc.)</DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="link-kind">Tipo</Label>
                <Select value={newLink.kind} onValueChange={(value) => setNewLink(prev => ({ ...prev, kind: value as EvidenceLinkKind }))}>
              ...
              <div className="space-y-2">
                ...
                <Input id="link-url" type="url" placeholder="https://..." value={newLink.url} onChange={(e) => setNewLink(prev => ({ ...prev, url: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="link-description">Descrição (opcional)</Label>
                <Input id="link-description" placeholder="Descrição do link" value={newLink.description} onChange={(e) => setNewLink(prev => ({ ...prev, description: e.target.value }))} />
              </div>
            </div>
            <DialogFooter>
            ...
            {evidenceLinks.map((link) => (
              <div key={link.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-sm">{link.description || EVIDENCE_LINK_KIND_LABELS[link.kind as EvidenceLinkKind]}</p>
                    <Badge variant="outline" className="text-xs mt-1">{EVIDENCE_LINK_KIND_LABELS[link.kind as EvidenceLinkKind]}</Badge>
                  </div>
                </div>
