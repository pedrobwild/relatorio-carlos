...
export function MeetingMinutesForm({ onComplete, initialData }: MeetingMinutesFormProps) {
  ...
  const onSubmit = (values: FormValues) => {
    ...
    const dateFormatted = new Date(values.meetingDate).toLocaleDateString('pt-BR');
    ...
    const participantsList = values.participants.map(p => `- ${p.name}${p.role ? ` (${p.role})` : ''}`).join('\n');
              ...
              render={({ field }) => (
                <FormItem className="flex items-start space-x-3 space-y-0">
                  ...
                  <div className="space-y-1 leading-none">
                    <FormLabel id="acknowledgement-description" className="font-medium">
                      Declaro ciente do que foi discutido e dos riscos/condições
                    </FormLabel>
                    <FormDescription>
                      Ao marcar esta opção, você confirma que compreende todos os pontos abordados na reunião.
                    </FormDescription>
                    <FormMessage />
                  </div>
                </FormItem>
