...
export function DigitalSignatureLog({
  formalizationId,
  signatures,
  parties,
  documentHash,
  lockedAt,
}: DigitalSignatureLogProps) {
  const { toast } = useToast();
  const [downloadingPartyId, setDownloadingPartyId] = useState<string | null>(
    null,
  );
  ...
  const handleDownloadCertificate = async (
    ...
    if (isDemo) {
      toast({
        ...
        description:
          "Certificados não estão disponíveis para dados de exemplo.",
        variant: "default",
      });
      return;
    }
    ...
    try {
      const { data, error } =
        await formalizationsRepo.downloadSignatureCertificate(
          formalizationId,
          partyId,
        );
    ...
    <Card>
      <CardHeader className="pb-3">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Registro de assinaturas eletrônicas com validade jurídica conforme Lei
          14.063/2020
        </p>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {documentHash && (
          <div className="p-3 bg-green-50/50 border border-green-200 rounded-lg">
            ...
            <div className="space-y-1.5 text-xs text-green-700">
              ...
              {lockedAt && (
                <p>
                  <span className="font-medium">Travado em:</span>{" "}
                  {formatDateTime(lockedAt).date} às{" "}
                  {formatDateTime(lockedAt).time}
                </p>
              )}
            </div>
          </div>
                  ...
                  <div className="flex-1 p-3 sm:p-4 bg-card border rounded-lg space-y-3">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="font-medium text-sm truncate">
                              {party?.display_name || "Desconhecido"}
                            </p>
                        ...
                        <Badge
                          variant="outline"
                          className="bg-green-50 text-green-700 border-green-200 shrink-0 text-[10px] px-2"
                        >
                          Assinado
                        </Badge>
                      </div>
        ...
        <div className="pt-4 border-t">
          <p className="text-[10px] text-muted-foreground leading-relaxed">
            <strong>Aviso Legal:</strong> Este registro de assinatura eletrônica
            possui validade jurídica nos termos da Lei nº 14.063/2020 e Medida
            Provisória nº 2.200-2/2001. A integridade do documento é garantida
            por hash criptográfico SHA-256. Os dados de identificação (IP,
            dispositivo, data/hora) são coletados automaticamente no momento da
            assinatura e servem como evidência de autenticidade.
          </p>
        </div>
      </CardContent>
    </Card>
