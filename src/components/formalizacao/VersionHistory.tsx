...
import { History, ChevronDown, ChevronRight, Eye, GitCompare, Calendar, User } from'lucide-react';
...
import { useFormalizationVersions, type FormalizationVersion } from'@/hooks/useFormalizationVersions';
...
function DiffDisplay({ oldText, newText, label }: { oldText: string; newText: string; label: string }) {
 ...
 <p className="text-sm text-muted-foreground/70 italic">Sem alterações</p>
 </div>
 );
 }
...
function VersionCard({ 
 ...
 const compareToTitle = isLatest ? currentTitle : (previousVersion?.title || version.title);
 const compareToSummary = isLatest ? currentSummary : (previousVersion?.summary || version.summary);
 const compareToBody = isLatest ? currentBodyMd : (previousVersion?.body_md || version.body_md);
 ...
 <span className="font-medium text-sm">Versão {version.version_number}</span>
 </button>
 ...
 <DialogHeader>
 <DialogTitle>Versão {version.version_number} - {version.title}</DialogTitle>
 </DialogHeader>
 ...
 v{version.version_number} → {isLatest ?'Atual' :`v${version.version_number + 1}`}
 </Badge>
 </div>
...
export function VersionHistory({ 
 ...
 const { data: versions, isLoading, error } = useFormalizationVersions(formalizationId);
