...
export function AttachmentsCard({ formalizationId, attachments }: AttachmentsCardProps) {
  ...
  const handlePreview = async (attachmentId: string, storagePath: string, filename: string, mimeType: string) => {
    ...
    if (!url) {
      toast({ title: 'Erro', description: 'Não foi possível carregar a pré-visualização do anexo.', variant: 'destructive' });
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div>
            <input ref={fileInputRef} type="file" className="hidden" onChange={handleFileSelect} accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.zip" />
            <Button variant="outline" size="sm" onClick={() => fileInputRef.current?.click()} disabled={uploading} aria-label="Adicionar anexo">
          ...
          {attachments.length === 0 ? (
            ...
            <div className="space-y-2">
              {attachments.map((attachment) => (
                <div key={attachment.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg hover:bg-muted/80 transition-colors">
                  <div className="flex items-center gap-3">
                    {getFileIcon(attachment.mime_type)}
                    <div>
                      <p className="font-medium text-sm">{attachment.original_filename}</p>
                      <p className="text-xs text-muted-foreground">{getFileTypeLabel(attachment.mime_type)} • {formatFileSize(attachment.size_bytes)}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="sm" onClick={() => handlePreview(attachment.id, attachment.storage_path, attachment.original_filename, attachment.mime_type)} aria-label={`Pré-visualizar ${attachment.original_filename}`}>
                      <Eye className="h-4 w-4 mr-1" />Ver
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleDownload(attachment.storage_path, attachment.original_filename)} aria-label={`Baixar ${attachment.original_filename}`}>
      ...
      <Dialog open={previewOpen} onOpenChange={(open) => { setPreviewOpen(open); if (!open) { setPreviewUrl(null); setPreviewFile(null); setPreviewingId(null); } }}>
        <DialogContent className="max-w-4xl w-[95vw] h-[85vh] p-0 flex flex-col gap-0">
          <DialogHeader className="p-4 border-b border-border shrink-0">
            <DialogTitle className="text-base truncate">{previewFile?.name ?? 'Pré-visualização do anexo'}</DialogTitle>
          </DialogHeader>
          <div className="flex-1 overflow-hidden">
            {previewingId ? (
              ...
              <DocumentViewer url={previewUrl} title={previewFile.name} mimeType={previewFile.mimeType} className="h-full rounded-none border-0" />
            ) : (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                <p className="text-muted-foreground">Não foi possível carregar a pré-visualização.</p>
                {previewFile && (
                  <Button onClick={() => handleDownload(previewFile.storagePath, previewFile.name)} variant="outline"><Download className="h-4 w-4 mr-2" />Baixar arquivo</Button>
                )}
              </div>
            )}
          </div>
