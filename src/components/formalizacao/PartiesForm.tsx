...
const schema = z.object({
  parties: z.array(partySchema).min(2, 'Adicione pelo menos um cliente e um representante da empresa'),
}).refine(
  (data) => data.parties.some(p => p.party_type === 'customer'),
  { message: 'Adicione pelo menos um cliente', path: ['parties'] }
).refine(
  (data) => data.parties.some(p => p.party_type === 'company'),
  { message: 'Adicione pelo menos um representante da empresa', path: ['parties'] }
);
...
export function PartiesForm({ onComplete, initialParties }: PartiesFormProps) {
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      parties: initialParties?.length ? initialParties : [
        { party_type: 'customer', display_name: '', email: '', role_label: 'Cliente', must_sign: true },
        { party_type: 'company', display_name: '', email: '', role_label: 'Responsável', must_sign: true },
      ],
    },
  });
  ...
  const customerParties = fields.filter((_, i) => form.watch(`parties.${i}.party_type`) === 'customer');
  const companyParties = fields.filter((_, i) => form.watch(`parties.${i}.party_type`) === 'company');
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {fields.map((field, index) => {
              if (form.watch(`parties.${index}.party_type`) !== 'customer') return null;
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Ex: Proprietário, Responsável" {...field} />
                        </FormControl>
                      </FormItem>
                    )}
                  />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="flex items-center space-x-2 space-y-0">
                        ...
                        <FormLabel className="font-normal">Deve assinar</FormLabel>
                      </FormItem>
                    )}
                  />
                </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {fields.map((field, index) => {
              if (form.watch(`parties.${index}.party_type`) !== 'company') return null;
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Função (opcional)</FormLabel>
                        <FormControl>
                          <Input placeholder="Ex: Gerente, Engenheiro" {...field} />
                        </FormControl>
                      </FormItem>
                    )}
                  />
