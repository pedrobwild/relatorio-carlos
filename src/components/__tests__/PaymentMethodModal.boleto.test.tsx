...
describe('PaymentMethodModal — fluxo de boleto + IA', () => {
  it('faz upload, invoca a edge function com base64 + mimeType e preenche boleto_code quando IA retorna 47+ dígitos', async () => {
    ...
    h.mockFunctionsInvoke.mockResolvedValue({ data: { code: aiCode, raw: aiCode }, error: null });
    ...
    await waitFor(() => {
      expect(h.mockFunctionsInvoke).toHaveBeenCalledWith('extract-boleto-code', {
        body: {
    ...
    await waitFor(() => {
      expect(h.mockToastSuccess).toHaveBeenCalledWith(expect.stringMatching(/extraído/i));
    });
  ...
  it('mostra aviso e preenche parcialmente quando IA retorna menos de 47 dígitos', async () => {
    ...
    await waitFor(() => {
      expect(h.mockToastWarning).toHaveBeenCalledWith(expect.stringMatching(/parcial/i));
    });
  ...
  it('exibe erro e não persiste quando a edge function falha', async () => {
    ...
    await waitFor(() => expect(h.mockFunctionsInvoke).toHaveBeenCalled());
    await waitFor(() => {
      expect(h.mockToastError).toHaveBeenCalledWith(expect.stringMatching(/extrair/i));
    });
    expect(h.mockFromUpdate).not.toHaveBeenCalledWith(expect.objectContaining({ boleto_code: expect.anything() }));
  });
