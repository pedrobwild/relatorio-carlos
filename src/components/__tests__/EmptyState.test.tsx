...
describe('EmptyState', () => {
  it('renders title correctly', () => {
    const { getByText } = render(<EmptyState title="Nenhum documento encontrado" />);
  ...
  it('renders description when provided', () => {
    ...
    expect(getByText('Envie seu primeiro documento para começar')).toBeInTheDocument();
  });
  ...
  it('applies compact styling when compact prop is true', () => {
    const { container } = render(
      <EmptyState title="Compacto" compact />
    );
  ...
  describe('variants', () => {
    const variants = ['documents', 'formalizations', 'schedule', 'purchases', 'payments', 'generic'] as const;
    
    variants.forEach((variant) => {
      it(`renders ${variant} variant without errors`, () => {
        const { getByText } = render(<EmptyState title={`${variant} empty state`} variant={variant} />);
