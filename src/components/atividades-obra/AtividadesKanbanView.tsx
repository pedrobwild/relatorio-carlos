...
export function AtividadesKanbanView({
  tasks,
  isLoading,
  onUpdateStatus,
  onDelete,
  ...
  const [dragOverColumn, setDragOverColumn] = useState<ObraTaskStatus | null>(
    null,
  );
      ...
      <div className="flex gap-3 overflow-x-auto pb-4 -mx-4 px-4 md:mx-0 md:px-0 md:grid md:grid-cols-4 min-h-[calc(100vh-220px)] scrollbar-hide">
        {TASK_STATUSES.map((col) => {
          ...
          return (
            ...
            >
              <div
                className={cn("px-2.5 py-2 rounded-t-xl", columnBg[col.value])}
              >
