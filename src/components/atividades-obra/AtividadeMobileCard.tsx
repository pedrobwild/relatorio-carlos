...
import { MoreVertical, Calendar, User, ChevronRight, AlertTriangle } from 'lucide-react';
...
const statusConfig: Record<ObraTaskStatus, { dot: string; bg: string; label: string }> = {
  pendente: { dot: 'bg-yellow-500', bg: 'bg-yellow-500/10', label: 'Pendente' },
  em_andamento: { dot: 'bg-blue-500', bg: 'bg-blue-500/10', label: 'Em andamento' },
  pausado: { dot: 'bg-orange-500', bg: 'bg-orange-500/10', label: 'Pausado' },
  concluido: { dot: 'bg-green-500', bg: 'bg-green-500/10', label: 'Concluído' },
};

export function AtividadeMobileCard({ task, responsibleName, onUpdateStatus, onDelete, onOpenDetail, onEdit }: Props) {
  ...
  const nextStatus = (): ObraTaskStatus | null => {
    switch (task.status) {
      ...
      case 'pausado': return 'em_andamento';
      default: return null;
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={cn('absolute left-0 top-3 bottom-3 w-1 rounded-full', config.dot)} />
        ...
        <div className="pl-4 pr-2 py-3">
          ...
          <div className="flex items-start gap-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 -mr-1 rounded-lg">
              ...
              <DropdownMenuContent align="end" className="min-w-[180px]">
                {TASK_STATUSES.map(s => (
                  ...
                  >
                    <div className={cn('w-2 h-2 rounded-full mr-2', statusConfig[s.value].dot)} />
                    {s.label}
                  </DropdownMenuItem>
                ))}
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            ...
            {isDone && task.days_overdue != null && (
              <span className={cn(
                ...
                task.days_overdue > 0 ? 'bg-destructive/10 text-destructive' : 'bg-green-500/10 text-green-600'
              )}>
                {task.days_overdue > 0 ? `${task.days_overdue}d atraso` : task.days_overdue === 0 ? '✓ Prazo' : `✓ ${Math.abs(task.days_overdue)}d antes`}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 mt-2.5">
            {next && (
              ...
              >
                <div className={cn('w-2 h-2 rounded-full', statusConfig[next].dot)} />
