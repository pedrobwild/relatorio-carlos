...
export function MeetingAvailabilityForm({
  ...
  const [selectedWeekdays, setSelectedWeekdays] = useState<string[]>(
    initialData?.preferred_weekdays ?? [],
  );
  ...
  const weekdayWarning = useMemo(() => {
    ...
    const outside = selectedWeekdays.filter(
      (wd) => !daysInRange.has(dayMap[wd]),
    );
    if (outside.length > 0)
      return "Alguns dias selecionados não estão dentro do intervalo escolhido.";
    return null;
  ...
  const validate = (): boolean => {
    ...
    if (startDate && endDate) {
      ...
      if (bdays === 0)
        errs.push("O intervalo deve conter pelo menos 1 dia útil.");
    }
    if (startDate) {
      const min = new Date(minDate);
      min.setHours(0, 0, 0, 0);
      ...
      s.setHours(0, 0, 0, 0);
      if (s < min)
        errs.push("Escolha uma data a partir de 2 dias úteis de hoje.");
    }
    if (selectedSlots.length === 0)
      errs.push("Selecione pelo menos um período de horário.");
    setErrors(errs);
    ...
    <Card>
      <CardContent className="pt-6 space-y-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Selecione um intervalo de datas e os horários do dia em que você
              pode. A equipe de Arquitetura agenda conforme a disponibilidade e
              confirma aqui no portal.
            </p>
          </div>
        </div>
            ...
            <li>
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="text-xs">
                      Dias úteis = segunda a sexta. Fins de semana não entram no
                      cálculo.
                    </p>
                  </TooltipContent>
                </Tooltip>
        ...
        <p className="text-xs text-muted-foreground -mt-2">
          Fins de semana aparecem bloqueados e não entram na contagem do
          período.
        </p>

        <div className="space-y-2">
          <label className="text-sm font-medium text-foreground">
            Quais dias da semana são melhores?{" "}
            <span className="text-muted-foreground font-normal">
              (opcional)
            </span>
          </label>
          <p className="text-xs text-muted-foreground -mt-1">
            Isso ajuda a equipe a priorizar dentro do seu intervalo.
          </p>
          <div className="flex flex-wrap gap-2">
            {WEEKDAYS.map(({ key, label }) => (
              <button
                ...
                type="button"
                className={cn(
                  "h-10 px-4 rounded-full border text-sm font-medium transition-colors min-h-[44px]",
                  selectedWeekdays.includes(key)
                    ? "bg-primary text-primary-foreground border-primary"
                    : "bg-background text-foreground border-border hover:bg-muted",
                )}
                onClick={() =>
                  toggleChip(key, selectedWeekdays, setSelectedWeekdays)
                }
              >
                {label}
              </button>
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-foreground">
            ...
            Em quais horários é melhor para você?
          </label>
          <p className="text-xs text-muted-foreground -mt-1">
          ...
          <div className="flex flex-wrap gap-2">
            {TIME_SLOTS.map(({ key, label }) => (
              <button
                ...
                onClick={() => {
                  toggleChip(key, selectedSlots, setSelectedSlots);
                  setErrors([]);
                }}
              >
                {label}
        ...
        <div className="flex gap-2">
          ...
          <Button
            className="h-12 min-h-[44px] flex-1 gap-2"
