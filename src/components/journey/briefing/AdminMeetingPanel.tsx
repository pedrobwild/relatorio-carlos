...
import { useMeetingAvailability, deriveMeetingState, type BriefingMeetingState } from '@/hooks/useMeetingAvailability';
...
const STATUS_BADGE: Record<BriefingMeetingState, { label: string; variant: 'secondary' | 'outline' | 'default' }> = {
  needs_availability: { label: '⏳ Aguardando disponibilidade', variant: 'secondary' },
  awaiting_scheduling: { label: '📋 Aguardando agendamento', variant: 'outline' },
...
export function AdminMeetingPanel({ stageId, projectId }: AdminMeetingPanelProps) {
        ...
        <CardContent className="pt-6 space-y-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-foreground">Reunião de Briefing</h3>
                <Badge variant={badge.variant} className="mt-1.5 text-xs">{badge.label}</Badge>
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 pl-11">
              ...
              {availability.notes && (
                <div className="sm:col-span-2">
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5">{availability.notes}</p>
                </div>
              )}
            </div>
          )}
