...
import { Edit2, ChevronDown, CheckCircle2, ImageIcon, Eye, FileText } from 'lucide-react';
...
import { CompleteStageButton, CompletedBadge } from './stage-detail/StageFooterActions';
  ...
  if (isBriefingStage) {
    return (
      <div className="space-y-6">
        {isEditing && <AdminEditForm stage={stage} projectId={projectId} onClose={() => setIsEditing(false)} />}
        {!isEditing && <BriefingStageLayout stage={stage} projectId={projectId} isAdmin={isAdmin} onStageCompleted={onStageCompleted} />}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          {isAdmin && !isEditing && (
            <Button size="sm" variant="outline" className="h-12 sm:h-10 gap-1.5 text-xs min-h-[44px] w-full sm:w-auto" onClick={() => setIsEditing(true)}>
              <Edit2 className="h-3.5 w-3.5" /> Editar etapa
            </Button>
          )}
          {canComplete && (
            <CompleteStageButton stageName={stage.name} stageId={stage.id} projectId={projectId} nextStageName={nextStageName} isPending={completeStage.isPending} onComplete={handleCompleteStage} />
          )}
  ...
  return (
    <div className="space-y-6">
      ...
      {(hasInstructions || isStaff) && (
        <InstructionsCollapsible hasInstructions={hasInstructions} displayHtml={displayHtml} isStaff={isStaff} onEditClick={() => setInstrEditorOpen(true)} />
      )}

      {/* Team for Mobilização */}
      {isMobilizacaoStage && (
        <MobilizacaoTeamCard
          members={members} isAdmin={isAdmin} teamExpanded={teamExpanded} setTeamExpanded={setTeamExpanded}
      ...
      {!isEditing && stage.cta_visible && stage.cta_text && (
        <div className="space-y-2">
          {stage.cta_text.toLowerCase().includes('reunião') ? (
            <MeetingCTA stageId={stage.id} stageName={stage.name} projectId={projectId} isAdmin={isAdmin} ctaText={stage.cta_text} />
          ) : (
            <Button className="w-full min-h-[44px]">{stage.cta_text}</Button>
          )}
          {stage.microcopy && <p className="text-xs text-muted-foreground">{stage.microcopy}</p>}
        </div>
      )}
      ...
      {!isProjetoExecutivoStage && !isMobilizacaoStage && (
        <>
          <StageDatesPanel stageId={stage.id} projectId={projectId} isAdmin={isAdmin} stageName={stage.name} />
          <Separator />
        </>
      )}
      ...
      {!isProjetoExecutivoStage && !isMobilizacaoStage && (
        <>
          <div className="grid gap-5 md:gap-6 md:grid-cols-2">
            <StageChecklist todos={stage.todos} owner="client" label="✔️ To-dos do Cliente" projectId={projectId} stageId={stage.id} isAdmin={isAdmin} />
            <StageChecklist todos={stage.todos} owner="bwild" label="🧰 To-dos Bwild" projectId={projectId} stageId={stage.id} isAdmin={isAdmin} />
          </div>
          <Separator />
        </>
      )}
          ...
          <div className="bg-card rounded-xl border border-border shadow-sm p-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => setVersionsOpen(true)} className="gap-1.5">
                <Eye className="h-4 w-4" /> {isStaff ? 'Gerenciar' : 'Visualizar'}
              </Button>
            </div>
          </div>
          <Separator />
      ...
      {!isProjetoExecutivoStage && !isMobilizacaoStage && (
        <>
          <StageRegistry stageId={stage.id} projectId={projectId} isAdmin={isAdmin} />
          <Separator />
        </>
      )}
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        ...
        {canComplete && isMobilizacaoStage && (
          <>
            <Button size="sm" className="h-12 sm:h-10 gap-1.5 text-xs min-h-[44px] w-full sm:w-auto sm:ml-auto" onClick={() => setMobilizacaoModalOpen(true)}>
              <CheckCircle2 className="h-3.5 w-3.5" /> Concluir etapa
            </Button>
            <MobilizacaoCompletionModal open={mobilizacaoModalOpen} onOpenChange={setMobilizacaoModalOpen} stageId={stage.id} projectId={projectId} onSuccess={() => onStageCompleted?.()} />
          </>
        )}

        {canComplete && !isMobilizacaoStage && (
          <CompleteStageButton stageName={stage.name} stageId={stage.id} projectId={projectId} nextStageName={nextStageName} isPending={completeStage.isPending} onComplete={handleCompleteStage} onSuccess={() => onStageCompleted?.()} />
        )}
      ...
      {/* Modals */}
      {isStaff && (
        <RichTextEditorModal open={instrEditorOpen} onOpenChange={setInstrEditorOpen} value={displayHtml} onSave={saveInstruction} title={`Editar Instruções — ${stage.name}`} />
      )}
      {isProjeto3DStage && <VersionsListModal projectId={projectId} open={versionsOpen} onOpenChange={setVersionsOpen} />}
      {isProjetoExecutivoStage && <ExecutivoVersionsModal projectId={projectId} open={executivoVersionsOpen} onOpenChange={setExecutivoVersionsOpen} />}
      {isMobilizacaoStage && (
        <TeamMemberEditModal open={teamEditModalOpen} onOpenChange={setTeamEditModalOpen} member={editingMember} onSave={handleSaveTeamMember} onUploadPhoto={handleUploadTeamPhoto} isSaving={isAdding || isUpdating} isUploading={isUploading} />
      )}
    </div>
  );
}
