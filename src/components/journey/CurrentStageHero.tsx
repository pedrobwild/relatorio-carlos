...
import {
  ChevronRight, Eye, Check, Circle, Lock, Clock,
...
type HeroVisualState = 'waiting_client' | 'in_progress' | 'validating' | 'completed' | 'next' | 'blocked';
...
const heroConfig: Record<HeroVisualState, {
  label: string;
  chipClass: string;
  ...
  waiting_client: {
    ...
    chipClass: 'bg-[hsl(var(--warning-light))] text-[hsl(var(--warning))] border-[hsl(var(--warning)/0.2)]',
    Icon: Clock,
  ...
  completed: {
    label: 'Concluída',
    chipClass: 'bg-[hsl(var(--success-light))] text-[hsl(var(--success))] border-[hsl(var(--success)/0.2)]',
    Icon: Check,
...
export function CurrentStageHero({ stage, projectId, onCtaClick }: CurrentStageHeroProps) {
  const { data: allDates, isLoading: datesLoading } = useStageDates(projectId, stage.id);
        ...
        {datesLoading ? (
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className={cn(
              ...
              {format(parseISO(nextDate.eff!), "dd 'de' MMMM", { locale: ptBR })}
            </span>
          </div>
        ) : null}
      </div>
