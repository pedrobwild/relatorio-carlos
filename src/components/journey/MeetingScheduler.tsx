 ...
 import { CalendarIcon, Clock, Plus, Trash2, Check, Loader2 } from 'lucide-react';
 ...
 import { useMeetingSlots, useAddMeetingSlot, useDeleteMeetingSlot, useBookMeetingSlot } from '@/hooks/useMeetingSlots';
 ...
 export function MeetingScheduler({ stageId, stageName, projectId, isAdmin, ctaText = 'Escolher data da reunião' }: MeetingSchedulerProps) {
   ...
   const handleBookSlot = (slotId: string, slotDatetime: string) => {
     bookSlot.mutate(
       ...
       {
         onSuccess: () => {
           toast.success('Reunião agendada com sucesso! A equipe foi notificada.');
           setOpen(false);
         },
   ...
   return (
     <Dialog open={open} onOpenChange={setOpen}>
       ...
       <DialogContent className="max-w-md">
         <DialogHeader>
           <DialogTitle>
             {isAdmin ? 'Gerenciar horários disponíveis' : 'Escolher data da reunião'}
           </DialogTitle>
         </DialogHeader>
               ...
               {isAdmin && (
                 <div className="space-y-3 p-4 bg-muted/30 rounded-lg">
                   <div className="flex items-center justify-between">
                     ...
                     {!isAddingSlot && (
                       <Button size="sm" variant="outline" onClick={() => setIsAddingSlot(true)}>
                   ...
                   {isAddingSlot && (
                     <div className="space-y-3">
                       ...
                       <div className="flex gap-2">
                         <Button size="sm" onClick={handleAddSlot} disabled={addSlot.isPending}>
                           {addSlot.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Adicionar'}
                         </Button>
                         <Button size="sm" variant="ghost" onClick={() => setIsAddingSlot(false)}>
                           Cancelar
                         </Button>
                       </div>
                     </div>
                       ...
                       return (
                         ...
                         >
                           ...
                           {isAdmin ? (
                             ...
                             <Button
                               size="sm"
                               onClick={() => handleBookSlot(slot.id, slot.slot_datetime)}
