...
const portalBullets = [
  { icon: '📍', text: 'Clareza do andamento: veja a etapa atual, o que já foi feito e o que vem a seguir.' },
  { icon: '✅', text: 'Decisões mais rápidas: aprove documentos e resolva pendências direto por aqui.' },
  { icon: '🗓️', text: 'Mais previsibilidade: acompanhe datas, reuniões e marcos importantes em um só lugar.' },
  { icon: '🔒', text: 'Mais segurança: decisões e arquivos ficam registrados, reduzindo dúvidas e ruídos.' },
  { icon: '💬', text: 'Contato fácil com o time: saiba quem é responsável por cada frente e como falar.' },
];
...
export function WelcomeGuideCard({ hero, onScrollToTeam }: WelcomeGuideCardProps) {
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-primary font-medium">Entenda como acompanhar seu projeto e aproveitar o portal</p>
          </div>
            ...
            <CardContent className="space-y-5 pt-0 px-4 pb-5 md:px-6 md:pb-6">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Aqui você acompanha cada etapa do seu projeto com transparência e previsibilidade. O portal reúne prazos, documentos, aprovações e comunicação com o time — para você tomar decisões com segurança e evitar retrabalho.
                </p>
              </div>

              <div className="space-y-3">
                <h4 className="text-sm font-semibold text-foreground">Como este portal te ajuda</h4>
              ...
              <div className="space-y-3">
                <h4 className="text-sm font-semibold text-foreground">Principais áreas do portal</h4>
                <ul className="space-y-1.5">
                  {portalAreas.map((area, i) => (
                    <li key={i} className="text-sm text-muted-foreground leading-relaxed">
                      <span className="font-medium text-foreground">{area.bold}:</span> {area.text}
                    </li>
                  ))}
                </ul>
              </div>
