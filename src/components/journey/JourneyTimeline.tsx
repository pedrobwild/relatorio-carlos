...
export function JourneyTimeline({ stages, activeStageId, onStageClick }: JourneyTimelineProps) {
  return (
    <div className="relative">
      {/* Vertical line */}
      <div className="absolute left-5 lg:left-4 top-0 bottom-0 w-0.5 bg-border" />
        ...
        {stages.map((stage) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative flex items-center gap-3 lg:gap-4 w-full p-3 rounded-lg text-left transition-all",
                "hover:bg-muted/50 active:bg-muted/70",
                "min-h-[56px]", // Ensure 44px+ touch target
            ...
            >
              ...
              <div
                className={cn(
                  "relative z-10 flex items-center justify-center w-10 h-10 lg:w-8 lg:h-8 rounded-full shrink-0",
                  config.bgColor
                )}
              >
                <Icon className={cn("h-5 w-5 lg:h-4 lg:w-4", config.color)} />
              </div>
