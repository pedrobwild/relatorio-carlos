...
export default function AtividadesObra() {
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (filterResponsible === "all") {
      /* no-op */
    } else if (filterResponsible === "unassigned")
      result = result.filter((t) => !t.responsible_user_id);
    else
      result = result.filter(
        (t) => t.responsible_user_id === filterResponsible,
      );

    if (searchQuery.trim()) {
      result = result.filter((t) =>
        matchesSearch(searchQuery, [t.title, t.description]),
      );
    }
    return result;
  ...
  const statusCounts = useMemo(() => {
    const pendente = filteredTasks.filter(
      (t) => t.status === "pendente",
    ).length;
    const em_andamento = filteredTasks.filter(
      (t) => t.status === "em_andamento",
    ).length;
    const pausado = filteredTasks.filter((t) => t.status === "pausado").length;
    const concluido = filteredTasks.filter(
      ...
      <PageContainer>
        <div className="flex items-center justify-between gap-2 mb-3">
          ...
          <Button
            ...
            size="sm"
            className="h-9 gap-1.5 rounded-lg font-semibold shrink-0"
          >
