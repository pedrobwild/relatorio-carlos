...
export default function JornadaProjeto() {
  ...
  const initialTab =
    tabFromUrl && validTabs.includes(tabFromUrl) ? tabFromUrl : "jornada";
  ...
  const hasRealProgress =
    journey?.stages?.some(
      (s) => s.status === "completed" || s.status === "waiting_action",
    ) ?? false;
  ...
  useEffect(() => {
    ...
    toast(
      `Bem-vindo ao portal, ${project.customer_name?.split(" ")[0] || ""}! 👋`,
      {
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            {journeyCopy.loading.initializing}
          </p>
        </div>
      </div>
    );
