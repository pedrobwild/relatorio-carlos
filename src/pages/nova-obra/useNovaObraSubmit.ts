...
export function useNovaObraSubmit() {
  ...
  const submit = async (
    ...
    const { error: engineerError } = await supabase
      ...
      .insert({ project_id: projectId, engineer_user_id: user.id, is_primary: true });
    if (engineerError) {
      console.error('Engineer assignment error:', engineerError);
      warnings.push({ step: 'Engenheiro', message: 'Não foi possível vincular engenheiro responsável' });
    }
    ...
    if (hasStudioData) {
      ...
      if (studioError) {
        ...
        warnings.push({ step: 'Imóvel', message: 'Dados do imóvel não foram salvos — edite na tela de Dados do Cliente' });
      }
    }
    ...
    if (createdUserId) {
      const { error: viewerError } = await supabase
        .from('project_members')
        .insert({ project_id: projectId, user_id: createdUserId, role: 'viewer' });
      if (viewerError) {
        console.error('Viewer assignment error:', viewerError);
        warnings.push({ step: 'Acesso cliente', message: 'Não foi possível vincular acesso do cliente ao projeto' });
      }
    }
    ...
    if (formData.is_project_phase) {
      const { error: journeyError } = await supabase
        .rpc('initialize_project_journey', { p_project_id: projectId });
      if (journeyError) {
        console.error('Journey initialization error:', journeyError);
        warnings.push({ step: 'Jornada', message: 'Jornada não foi inicializada — pode ser criada depois' });
      }
    }
    ...
    if (selectedTemplate && Array.isArray(selectedTemplate.default_activities) && selectedTemplate.default_activities.length > 0) {
      ...
      if (actError) {
        ...
        warnings.push({ step: 'Atividades', message: 'Atividades do template não foram criadas' });
      }
      ...
      if (validActivities.length > 0) {
        ...
        if (actError) {
          ...
          warnings.push({ step: 'Atividades', message: 'Atividades manuais não foram criadas' });
        }
      }
    }
    ...
    if (numInstallmentsParsed > 0) {
      ...
      if (paymentError) {
        ...
        warnings.push({ step: 'Parcelas', message: 'Parcelas de pagamento não foram criadas' });
      }
    }
    ...
    if (budgetFile) {
      ...
      if (uploadError) {
        ...
        warnings.push({ step: 'Orçamento', message: 'Arquivo de orçamento não foi enviado' });
      } else {
        ...
        if (docError) {
          ...
          warnings.push({ step: 'Orçamento', message: 'Registro do orçamento não foi criado' });
        }
    ...
    if (contractFile) {
      ...
      if (!uploadError) {
        ...
        if (docError) {
          ...
          warnings.push({ step: 'Contrato', message: 'Registro do contrato não foi criado' });
        }
      } else {
        console.error('Contract upload error:', uploadError);
        warnings.push({ step: 'Contrato', message: 'Arquivo do contrato não foi enviado' });
      }
    }
    ...
    return { project: { id: projectId, name: formData.name.trim() }, createdUserId, warnings };
  };
