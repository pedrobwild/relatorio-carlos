...
import type { ProjectTemplate, TemplateActivity, TemplateCustomField } from '@/hooks/useProjectTemplates';
...
export function TemplateSelectorCard({
  ...
  formData, onFormChange, customFieldValues, onCustomFieldChange,
      ...
      <CardContent>
        {cats.length > 1 && (
          <div className="flex gap-1.5 flex-wrap mb-3">
            <Badge variant={templateCategoryFilter === '__all__' ? 'default' : 'outline'} className="cursor-pointer" onClick={() => setTemplateCategoryFilter('__all__')}>Todos</Badge>
            {cats.map(c => (
              <Badge key={c} variant={templateCategoryFilter === c ? 'default' : 'outline'} className="cursor-pointer" onClick={() => setTemplateCategoryFilter(c)}>
        ...
        <Select
          onValueChange={(id) => {
            ...
            if (tpl) {
              ...
              if (tpl.default_contract_value) onFormChange('contract_value', tpl.default_contract_value.toString());
              if (formData.planned_start_date && Array.isArray(tpl.default_activities) && tpl.default_activities.length > 0) {
                const total = (tpl.default_activities as TemplateActivity[]).reduce((s, a) => s + a.durationDays, 0);
                if (total > 0) {
        ...
        {selectedTemplate && Array.isArray(selectedTemplate.default_activities) && selectedTemplate.default_activities.length > 0 && (
          <div className="mt-3 space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              <Badge variant="secondary">{selectedTemplate.default_activities.length} atividades</Badge>
              ...
              {selectedTemplate.default_activities.reduce((s, a) => s + (a as TemplateActivity).weight, 0) > 0 && (
                <Badge variant="outline">{selectedTemplate.default_activities.reduce((s, a) => s + (a as TemplateActivity).weight, 0)}% peso total</Badge>
              )}
            </div>
                    ...
                    <div key={i} className="flex items-center gap-2 group/bar">
                      ...
                      <div className="flex-1 h-4 relative rounded-sm overflow-hidden bg-muted">
                        <div
                          ...
                          style={{ left: `${(cumDays[i] / total) * 100}%`, width: `${Math.max((act.durationDays / total) * 100, 2)}%` }}
                ...
                {(selectedTemplate.custom_fields as TemplateCustomField[]).map((field) => (
                  <div key={field.key} className="space-y-1">
                    ...
                    {field.type === 'select' && field.options ? (
                      <Select value={customFieldValues[field.key] ?? ''} onValueChange={(v) => onCustomFieldChange({ ...customFieldValues, [field.key]: v })}>
                      ...
                      <Input
                        ...
                        onChange={(e) => onCustomFieldChange({ ...customFieldValues, [field.key]: e.target.value })}
