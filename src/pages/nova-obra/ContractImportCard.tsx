...
import { FileUp, Loader2, CheckCircle2, AlertTriangle, X, Sparkles, FileText } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { cn } from '@/lib/utils';
import { supabase } from '@/integrations/supabase/client';
import type { FormData, ContractImportState, ContractParseResult, ContractConflict } from './types';
...
export function ContractImportCard({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (!response.ok) {
        const err = await response.json().catch(() => ({ error: 'Falha na análise' }));
        ...
        {!isSuccess && !isProcessing && (
          <div
            className={cn(
              ...
              dragActive ? 'border-primary bg-primary/10' : 'border-muted-foreground/20 hover:border-primary/50',
            )}
          ...
          >
            ...
            <p className="text-sm font-medium">Arraste o PDF do contrato aqui</p>
            <p className="text-xs text-muted-foreground mt-1">ou clique para selecionar · PDF até 20MB</p>
            <input
              ref={fileInputRef}
              type="file"
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-sm font-medium truncate">{contractState.aiSourceDocumentName}</span>
                <Badge variant="secondary" className="text-[10px] shrink-0">
                  <CheckCircle2 className="h-3 w-3 mr-1" />
                  Analisado
                </Badge>
              </div>
              <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" onClick={handleClear} title="Remover contrato">
            ...
            {/* Conflicts detail */}
            {hasConflicts && (
              <div className="rounded-lg border border-amber-500/30 bg-amber-50/50 dark:bg-amber-950/20 p-3 space-y-1.5">
                <p className="text-xs font-medium text-amber-700 dark:text-amber-400">
                  Encontramos possíveis divergências no contrato. Revise os campos destacados.
                </p>
