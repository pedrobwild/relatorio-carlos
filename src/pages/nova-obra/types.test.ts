...
describe('Nova Obra Types', () => {
  describe('formSchema validation', () => {
    ...
    it('rejects missing name', () => {
      const data = { ...initialFormData, name: '', customer_name: 'João', customer_email: 'j@e.com', create_user: false };
      const result = formSchema.safeParse(data);
      expect(result.success).toBe(false);
    });

    it('rejects invalid email', () => {
      const data = { ...initialFormData, name: 'X', customer_name: 'J', customer_email: 'invalid', create_user: false };
      const result = formSchema.safeParse(data);
      expect(result.success).toBe(false);
    });
