...
export default function AdminResearch() {
 ...
 const [history, setHistory] = useState<Array<{ query: string; content: string; citations: string[] }>>([]);
 ...
 <Badge variant="secondary" className="ml-auto text-xs">Powered by Perplexity</Badge>
 </div>
 </div>
 ...
 <CardDescription>
 Use IA para pesquisar funcionalidades de softwares de gestão de obras do mercado
 </CardDescription>
 </CardHeader>
 ...
 <ToggleGroup type="single" value={searchFocus} onValueChange={(v) => v && setSearchFocus(v)} size="sm">
 ...
 <ToggleGroupItem value="international" className="text-xs gap-1.5">
 ...
 <p className="text-sm text-muted-foreground font-medium">Sugestões de pesquisa:</p>
 ...
 <p className="text-sm text-muted-foreground">Pesquisando referências de mercado...</p>
 </CardContent>
 </Card>
 )}
 ...
 <p className="text-xs font-medium text-muted-foreground mb-2">Fontes ({result.citations.length})</p>
 ...
 {history.length > 1 && (
 <div className="space-y-3">
 <p className="text-sm text-muted-foreground font-medium">Pesquisas anteriores:</p>
 <div className="space-y-2">
 {history.slice(1).map((h, i) => (
 <button
 key={i}
 onClick={() => { setQuery(h.query); setResult({ content: h.content, citations: h.citations }); }}
