...
export default function GestaoAtividades() {
  ...
  const responsibleOptions = useMemo(() => {
    const ids = new Set(
      tasks.map((t) => t.responsible_user_id).filter(Boolean) as string[],
    );
  ...
  const filtered = useMemo(() => {
    ...
    if (filterProject !== "all")
      result = result.filter((a) => a.project_id === filterProject);
    if (filterResponsible !== "all") {
      if (filterResponsible === "unassigned")
        result = result.filter((a) => !a.responsible_user_id);
      else
        result = result.filter(
          (a) => a.responsible_user_id === filterResponsible,
        );
    }
  ...
  return (
    <PageContainer
      maxWidth="full"
      className="py-4 sm:py-6 flex flex-col flex-1 min-h-0"
    >
      {/* ── Header ── */}
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">
              Visão unificada de todas as obras
            </p>
          </div>
          {/* View toggle */}
          <div
            className="flex items-center rounded-lg border border-border/40 bg-muted/30 p-0.5 shrink-0"
            role="radiogroup"
          >
            {[
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 -mx-4 px-4 md:mx-0 md:px-0 md:grid md:grid-cols-4 min-h-[calc(100vh-220px)] scrollbar-hide">
          {COLUMNS.map((col) => {
            ...
            return (
              ...
              >
                <div
                  className={cn("px-2.5 py-2 rounded-t-xl", columnBg[col.key])}
                >
                ...
                <div className="p-1.5 space-y-1.5 flex-1 bg-muted/20 rounded-b-2xl overflow-y-auto">
                  {colTasks.map((task) => {
                    const responsible = getMemberName(
                      staffUsers,
          ...
          <div className="hidden md:block border rounded-lg overflow-hidden">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead
                    className="w-[22%] cursor-pointer select-none"
                  ...
                  >
                    <span className="inline-flex items-center gap-1">
                      Obra <SortIcon field="project_name" />
                    </span>
                  </TableHead>
                  <TableHead
                    className="cursor-pointer select-none"
