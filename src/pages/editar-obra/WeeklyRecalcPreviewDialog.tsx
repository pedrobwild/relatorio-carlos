...
function formatDate(iso: string | null) {
  if (!iso) return "—";
  try {
    return format(parseISODateLocal(iso), "dd 'de' MMM 'de' yyyy", {
      locale: ptBR,
    });
  } catch {
    return iso;
...
export function WeeklyRecalcPreviewDialog({
  ...
  const newEndDate =
    preview.length > 0 ? preview[preview.length - 1].planned_end : null;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Revise as etapas reorganizadas em semanas úteis (Seg→Sex) antes de
            aplicar.
          </DialogDescription>
        </DialogHeader>
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
                    <span className={changed ? "line-through" : ""}>
                      {formatDate(a.planned_start)} →{" "}
                      {formatDate(a.planned_end)}
                    </span>
                    {changed && r && (
                      <>
                        <ArrowRight className="h-3 w-3" />
                        <span className="text-foreground font-medium">
                          {formatDate(r.planned_start)} →{" "}
                          {formatDate(r.planned_end)}
                        </span>
                      </>
                    )}
                  </div>
        ...
        <DialogFooter className="gap-2 sm:gap-2">
          ...
          >
            {changedCount === 0
              ? "Nada para aplicar"
              : `Aplicar a ${changedCount} etapa(s)`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function Summary({
  ...
  return (
    <div className="min-w-0">
      ...
      <p
        className={`text-sm font-medium truncate ${highlight ? "text-primary" : ""}`}
      >
        {value}
      </p>
      {hint && (
