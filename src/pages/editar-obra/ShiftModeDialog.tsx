...
import {
  shiftActivityDates,
  type ShiftMode,
  type ShiftableActivity,
} from "@/lib/shiftActivityDates";
...
export function ShiftModeDialog({
  ...
  const preview = useMemo(() => {
    ...
    const { changedIds } = shiftActivityDates(
      activities,
      oldStart,
      oldEnd,
      newStart,
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Você alterou as datas planejadas do projeto. Como deseja atualizar
            as {activityCount} atividade(s) do cronograma?
          </AlertDialogDescription>
        </AlertDialogHeader>

        <RadioGroup
          value={mode}
          onValueChange={(v) => setMode(v as ShiftMode)}
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-tiny text-muted-foreground mt-1">
                Desloca todas as atividades a partir do novo início. Cada
                atividade conserva sua duração original. O novo término do
                projeto é ignorado.
              </p>
            </div>
          </div>
              ...
              >
                ...
                {onlyStart && (
                  <span className="text-tiny text-muted-foreground font-normal">
                    {" "}
                    (igual ao anterior se só o início mudou)
                  </span>
                )}
              </Label>
              <p className="text-tiny text-muted-foreground mt-1">
                Escala as durações das atividades para preencher exatamente o
                intervalo entre o novo início e o novo fim do projeto.
              </p>
            </div>
          </div>
        </RadioGroup>
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <span className="text-foreground">
                    {formatShiftDays(preview.shiftDays)}
                  </span>
                </div>
              </div>
              <div className="flex items-start gap-2">
