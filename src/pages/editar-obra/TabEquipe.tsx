...
export function TabEquipe({
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><UserPlus className="h-5 w-5" />Adicionar Membro</CardTitle>
          <CardDescription>Selecione um engenheiro disponível para adicionar à equipe do projeto</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              <Select value={selectedEngineer} onValueChange={setSelectedEngineer}>
                <SelectTrigger><SelectValue placeholder="Selecione um engenheiro..." /></SelectTrigger>
                <SelectContent>
                  {availableEngineers.length > 0 ? availableEngineers.map((eng) => (
                    ...
                    <SelectItem value="_empty" disabled>Nenhum engenheiro disponível</SelectItem>
                  )}
                </SelectContent>
              </Select>
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Equipe atribuída ao projeto com suas funções</CardDescription>
        </CardHeader>
        <CardContent>
          {members.length > 0 ? (
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>{member.user_name || member.user_email} será removido do projeto.</AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel>Cancelar</AlertDialogCancel>
                          <AlertDialogAction onClick={() => onRemoveMember(member.id)}>Remover</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  </div>
            ...
            <p className="text-center text-muted-foreground py-8">Nenhum membro adicionado. Use o seletor acima para adicionar.</p>
          )}
        </CardContent>
      </Card>
      ...
      {engineers.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Engenheiros atribuídos pelo sistema antigo</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Link2 className="h-5 w-5" />Acesso do Cliente</CardTitle>
          <CardDescription>Vincule o cliente à obra para que ele acesse o portal</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {customer ? (
                ...
                <div className="flex items-center gap-2">
                  {customer.customer_user_id ? (
                    <Badge className="bg-[hsl(var(--success))]/10 text-[hsl(var(--success))] border-[hsl(var(--success))]/20">
                      <CheckCircle2 className="h-3 w-3 mr-1" />Acesso vinculado
                    </Badge>
                  ) : (
                    <CustomerLinkButton customer={customer} projectId={projectId} onLinked={onCustomerLinked} />
                  )}
                </div>
              </div>
              {!customer.customer_user_id && (
            ...
            <AddCustomerSection projectId={projectId} onAdded={onCustomerAdded} />
          )}
        </CardContent>
      </Card>
    </div>
...
function CustomerLinkButton({ customer, projectId, onLinked }: { customer: Customer; projectId: string; onLinked: (c: Customer) => void }) {
  ...
  const handleLink = async () => {
    setLinking(true);
    try {
      const { data: profile, error: profileError } = await supabase.from('profiles').select('user_id').eq('email', customer.customer_email.toLowerCase().trim()).maybeSingle();
      if (profileError) throw profileError;
      if (!profile) {
        toast({ title: 'Usuário não encontrado', description: `Nenhuma conta encontrada para ${customer.customer_email}. O cliente precisa criar uma conta primeiro.`, variant: 'destructive' });
        return;
      }
      const { error: updateError } = await supabase.from('project_customers').update({ customer_user_id: profile.user_id }).eq('id', customer.id);
...
function AddCustomerSection({ projectId, onAdded }: { projectId: string; onAdded: (c: Customer) => void }) {
  ...
  const handleAdd = async () => {
    ...
    setAdding(true);
    try {
      const { data: profile } = await supabase.from('profiles').select('user_id').eq('email', email.toLowerCase().trim()).maybeSingle();
      const { data: newCustomer, error } = await supabase.from('project_customers').insert({
      ...
      onAdded(newCustomer);
      toast({ title: 'Cliente adicionado!', description: profile?.user_id ? `${name} foi adicionado e já possui acesso ao portal.` : `${name} foi adicionado. O acesso será vinculado quando ele fizer login.` });
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Erro';
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">Nenhum cliente vinculado a esta obra. Adicione um cliente para que ele possa acessar o portal.</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="sm:col-span-2"><Label>Nome completo *</Label><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome do cliente" /></div>
        <div><Label>E-mail *</Label><Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="cliente@email.com" /></div>
