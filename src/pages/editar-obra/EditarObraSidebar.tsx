...
interface EditarObraSidebarProps {
  ...
  customer: { customer_name: string | null; customer_email: string | null; customer_phone: string | null } | null;
}

export function EditarObraSidebar({ project, studioInfo, customer }: EditarObraSidebarProps) {
  const hasAddress = studioInfo.endereco_completo || studioInfo.cidade || studioInfo.bairro;
  const hasStudioData = studioInfo.tamanho_imovel_m2 || studioInfo.data_recebimento_chaves || studioInfo.tipo_de_locacao;
          ...
          <CardContent className="p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Badge variant="outline" className={`${statusColors[project.status]} cursor-help`}>
            ...
            {project.is_project_phase && (
              <div className="flex items-center justify-between">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Badge variant="secondary" className="cursor-help">Projeto</Badge>
                  </TooltipTrigger>
                  <TooltipContent>
                    <p className="text-xs">Fase de planejamento e projeto — ainda não iniciou execução</p>
                  </TooltipContent>
                </Tooltip>
              </div>
            )}
            ...
            <CardContent className="p-4 pt-0 space-y-2.5">
              {hasAddress && (
                <div className="flex items-start gap-2 text-sm">
                  ...
                  <span className="text-foreground leading-snug">
                    {[studioInfo.endereco_completo, studioInfo.bairro, studioInfo.cidade]
                      .filter(Boolean)
                      .join(', ')}
                  </span>
                </div>
              ...
              {studioInfo.tipo_de_locacao && (
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <span className="text-xs bg-muted px-2 py-0.5 rounded-full">{studioInfo.tipo_de_locacao}</span>
                </div>
              )}
              {studioInfo.data_recebimento_chaves && (
                <div className="flex items-center gap-2 text-sm">
                  <Key className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                  <span>Chaves: {format(parseLocalDate(studioInfo.data_recebimento_chaves), 'dd/MM/yyyy', { locale: ptBR })}</span>
                </div>
              )}
            </CardContent>
          </Card>
