...
function ComprasTabContent({ purchaseType }: { purchaseType: PurchaseType }) {
    ...
    try {
      ...
      if (count > 0) {
        ...
        queryClient.invalidateQueries({
          queryKey: queryKeys.purchases.list(state.projectId),
        });
      } else {
  ...
  const orderedCount = useMemo(
    () =>
      state.filteredPurchases.filter(
  ...
  const availableSubcategories = getSubcategoriesByType(
    isProduto ? "produtos" : "prestadores",
  );
  ...
  return (
    <div className="space-y-5 py-4">
      ...
      {purchaseType === "prestador" && (
        <PrestadorCalendar
          onNew={() => state.handleOpenDialog()}
          onBulkNew={() => setBulkOpen(true)}
        />
      )}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Select
          ...
          onValueChange={state.setFilterStatus}
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="ordered">
              {isProduto ? "Pedido" : "Contratado"}
            </SelectItem>
            <SelectItem value="in_transit">
              {isProduto ? "Em Trânsito" : "Em Execução"}
            </SelectItem>
            <SelectItem value="delivered">
              {isProduto ? "Entregue" : "Concluído"}
            </SelectItem>
            {isProduto && (
              <SelectItem value="sent_to_site">Enviado p/ Obra</SelectItem>
            )}
            <SelectItem value="cancelled">Cancelado</SelectItem>
          </SelectContent>
        </Select>
        <Select
          value={state.filterActivity}
          onValueChange={state.setFilterActivity}
        >
        ...
        {availableSubcategories.length > 0 && (
          <Select
            value={state.filterSubcategory}
            onValueChange={state.setFilterSubcategory}
          >
      ...
      <PurchaseFormDialog
        ...
        isSubmitting={
          state.addPurchase.isPending || state.updatePurchase.isPending
        }
