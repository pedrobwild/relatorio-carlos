...
import { getStatusBadgeVariant, type AttachmentRow, type EvidenceLinkRow } from './formalizacao-detalhe/types';
...
import { SignatureBlock, MobileSignatureBar } from './formalizacao-detalhe/SignatureBlock';
...
export default function FormalizacaoDetalhe() {
  ...
  const {
    id, formalizacao, isLoading, isAdmin, isDraft, hasParties,
    parties, acknowledgements, events, activeTab, setActiveTab,
    acknowledged, setAcknowledged, downloadingPdf, sendingForSignature,
    editingContent, setEditingContent, pendingPartyForUser,
    goBackToList, handleSendForSignature, handleAcknowledge,
    handleDownloadPdf, handleDelete, handleUpdateContent,
    deleteFormalizacao, acknowledge,
  } = state;
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <Card className="max-w-sm mx-4">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-6">Esta formalização não existe ou você não tem acesso.</p>
            <Button onClick={goBackToList} className="w-full">Voltar para lista</Button>
          </CardContent>
        </Card>
      </div>
    );
              ...
              <CardContent className="p-6">
                {isAdmin && acknowledgements.length === 0 && (
                  <div className="flex justify-end mb-3">
                    <Button variant="outline" size="sm" onClick={() => setEditingContent(true)} className="gap-1.5">
                ...
                {(() => {
                  ...
                  const proseClasses = "prose prose-sm dark:prose-invert max-w-none prose-headings:text-foreground prose-headings:font-bold prose-h1:text-xl prose-h2:text-lg prose-h3:text-base prose-p:text-muted-foreground prose-p:leading-relaxed prose-strong:text-foreground prose-li:text-muted-foreground prose-hr:border-border prose-hr:my-6 [&>*+*]:mt-4 [&>h1]:mt-0 [&>h2]:mt-6 [&>h3]:mt-5 [&>p]:mt-2 text-justify";
                  if (isHtml) {
                    return <div className={proseClasses} dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(content) }} />;
                  }
                  return <div className={proseClasses}><ReactMarkdown>{content}</ReactMarkdown></div>;
                })()}
              </CardContent>
            </Card>
