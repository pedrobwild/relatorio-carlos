...
import { ArrowLeft, ArrowRight, Send, Check, Loader2, Rocket } from "lucide-react";
...
export default function EditarObraWizard() {
  ...
  const {
    ...
    error: loadError,
    projectName,
  } = useEditProjectLoader(projectId);
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (projectError)
        throw new Error("Falha ao atualizar projeto:" + projectError.message);
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-amber-800">
              Rascunho — Recebido via integração
            </p>
            <p className="text-[11px] text-amber-600">
              Revise as informações e clique em"Publicar Obra" para ativar o
              projeto.
            </p>
          </div>
        </motion.div>
                ...
                <div>
                  {isLastStep ? (
                    <Button
                      type="submit"
                      disabled={saving}
                      className="min-h-[44px] bg-gradient-to-r from-primary to-[hsl(var(--primary-dark))]"
                    >
                      {saving ? (
                        <>
