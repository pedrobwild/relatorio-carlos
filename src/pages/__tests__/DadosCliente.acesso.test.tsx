  ...
  const from = (table: string) => {
      ...
      return {
        select: () => ({
          eq: (_col: string, val: string) => ({
            maybeSingle: async () => ({ data: studioStore[val] ?? null, error: null }),
          }),
        }),
        upsert: (payload: any) => {
          upsertSpy(payload);
    ...
    if (table === 'projects') {
      return {
        select: () => ({
          eq: () => ({
            single: async () => ({ data: { name: 'Obra Teste', unit_name: null }, error: null }),
          }),
        }),
      };
    }
...
describe('DadosCliente · Acesso à obra (persistência)', () => {
  ...
  it('persiste Local da chave e Senha ao salvar e recarrega ao remontar', async () => {
    ...
    fireEvent.change(keyInput, { target: { value: 'Portaria — falar com Sr. João' } });
