...
const obrasFixture: PainelObra[] = [
  makeObra({ id: 'o-s1', customer_name: 'Cliente S1', etapa: 'Execução', inicio_etapa: '2026-04-25' }), // 4 dias → S1
  makeObra({ id: 'o-s2', customer_name: 'Cliente S2', etapa: 'Execução', inicio_etapa: '2026-04-22' }), // 7 dias → S2
  makeObra({ id: 'o-s3', customer_name: 'Cliente S3', etapa: 'Execução', inicio_etapa: '2026-04-15' }), // 14 dias → S3
  makeObra({ id: 'o-s5', customer_name: 'Cliente S5', etapa: 'Execução', inicio_etapa: '2026-04-01' }), // 28 dias → S5
  makeObra({ id: 'o-plan',  customer_name: 'Cliente Plan',  etapa: 'Planejamento' }),
  makeObra({ id: 'o-med',   customer_name: 'Cliente Med',   etapa: 'Medição' }),
  makeObra({ id: 'o-final', customer_name: 'Cliente Final', etapa: 'Finalizada' }),
];
...
describe('PainelObras — paridade rótulo renderizado × formatEtapaLabel', () => {
  it('tabela: cada linha exibe exatamente o que formatEtapaLabel retorna', () => {
    ...
    const desktopTh = container.querySelector('[data-testid="painel-obras-th-cliente"]');
  ...
  it('board: cada grupo de Execução tem label === formatEtapaLabel da obra', () => {
    ...
    for (const obra of execObras) {
      ...
      const found = labels.find((l) => l === expected || l.startsWith(`${expected} `));
      expect(
        found,
    ...
    for (const etapa of ['Planejamento', 'Medição', 'Finalizada'] as const) {
      const found = labels.find((l) => l === etapa || l.startsWith(`${etapa} `));
