...
import {
  ArrowLeft, Loader2, Calendar, User, Building2, Clock,
  FileText, ExternalLink, Send, AlertTriangle,
  Link as LinkIcon, MapPin, Ruler, Package, DollarSign,
...
const STATUSES: Record<string, { label: string; className: string }> = {
  ...
  in_progress: { label: 'Em Produção', className: 'bg-primary/10 text-primary' },
  review: { label: 'Revisão', className: 'bg-[hsl(var(--warning))]/10 text-[hsl(var(--warning))]' },
  waiting_info: { label: 'Aguardando Info', className: 'bg-[hsl(var(--warning))]/10 text-[hsl(var(--warning))]' },
  blocked: { label: 'Bloqueado', className: 'bg-destructive/10 text-destructive' },
  ready: { label: 'Pronto', className: 'bg-[hsl(var(--success))]/10 text-[hsl(var(--success))]' },
  sent_to_client: { label: 'Enviado ao Cliente', className: 'bg-[hsl(var(--success))]/10 text-[hsl(var(--success))]' },
  approved: { label: 'Aprovado', className: 'bg-[hsl(var(--success))]/10 text-[hsl(var(--success))]' },
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              {budget.sequential_code && (
                <span className="text-xs font-mono text-muted-foreground">{budget.sequential_code}</span>
              )}
              <h1 className="text-lg font-semibold truncate">{budget.project_name}</h1>
            </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {budget.demand_context && (
            <Card>
              ...
              <CardContent>
                <p className="text-sm whitespace-pre-wrap leading-relaxed">{budget.demand_context}</p>
              </CardContent>
            </Card>
          )}
          ...
          {budget.briefing && (
            <Card>
              ...
              <CardContent>
                <p className="text-sm whitespace-pre-wrap leading-relaxed">{budget.briefing}</p>
              </CardContent>
            </Card>
          )}
          ...
          {budget.internal_notes && (
            <Card className="border-[hsl(var(--warning))]/20 bg-[hsl(var(--warning))]/5">
              ...
              <CardContent>
                <p className="text-sm whitespace-pre-wrap leading-relaxed">{budget.internal_notes}</p>
              </CardContent>
            </Card>
          )}
          ...
          {/* Empty state */}
          {!budget.briefing && !budget.demand_context && !budget.internal_notes && links.length === 0 && (
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              {(budget.bairro || budget.city) && (
                <InfoRow icon={<MapPin className="h-3.5 w-3.5" />} label="Local" value={[budget.bairro, budget.city].filter(Boolean).join(', ')} />
              )}
              {budget.condominio && (
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              {budget.updated_at && (
                <InfoRow icon={<Calendar className="h-3.5 w-3.5" />} label="Atualizado" value={format(new Date(budget.updated_at), 'dd/MM/yyyy HH:mm')} />
              )}
              {budget.due_at && (
                  ...
                  return (
                    <div key={ev.id} className="flex gap-3">
                      <div className="flex flex-col items-center">
                        <div className={`h-2.5 w-2.5 rounded-full mt-1.5 shrink-0 ${ev.event_type === 'comment' ? 'bg-primary/60' : 'bg-primary'}`} />
                      ...
                      <div className="pb-4 min-w-0">
                        <p className="text-xs leading-snug">
                          ...
                          {ev.event_type === 'status_change' && statusTo
                            ? <> alterou status para <span className="font-medium">{statusTo.label}</span></>
...
function MiniStat({ label, value, icon, accent }: { label: string; value: string; icon: React.ReactNode; accent?: boolean }) {
  return (
    <div className="space-y-0.5">
      ...
      <p className={`text-sm font-semibold tabular-nums ${accent ? 'text-primary' : ''}`}>{value}</p>
    </div>
  );
}
      ...
      <CollapsibleTrigger className="w-full">
        <div className="flex items-center justify-between p-3 rounded-lg border border-border hover:bg-muted/30 transition-colors cursor-pointer">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-sm font-medium truncate">{section.title || 'Seção'}</span>
            {section.is_optional && (
              <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-[hsl(var(--warning))]/10 text-[hsl(var(--warning))] shrink-0">Opcional</span>
            )}
            <span className="text-xs text-muted-foreground shrink-0">({section.itemRows.length} itens)</span>
          </div>
          <div className="flex items-center gap-4 text-xs tabular-nums shrink-0">
            ...
            <span className="font-semibold">Venda: {formatBRL(section.secSale)}</span>
          </div>
        </div>
      </CollapsibleTrigger>
      <CollapsibleContent>
