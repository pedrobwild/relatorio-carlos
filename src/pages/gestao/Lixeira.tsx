...
import {
  statusColors,
  statusLabels,
} from "@/components/admin/obras/obraCardUtils";
...
export default function Lixeira() {
  ...
  const filtered = useMemo(() => {
    let rows: (ProjectWithCustomer & { deleted_at?: string | null })[] =
      (data ?? []) as any;
    ...
    if (rangeFilter !== "all") {
      ...
      rows = rows.filter(
        (p) => p.deleted_at && new Date(p.deleted_at).getTime() >= cutoff,
      );
    }
    rows = [...rows].sort((a, b) => {
    ...
    <div className="space-y-4 p-4 md:p-6">
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Obras movidas para a lixeira. Restaure para reativar ou exclua
          definitivamente.
        </p>
      </header>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          <SelectTrigger
            className="w-[180px]"
            aria-label="Filtrar por período de exclusão"
          >
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-[200px] text-right text-xs">
                  Ações
                </TableHead>
              </TableRow>
            </TableHeader>
                      ...
                      {deletedAt ? (
                        <div className="flex flex-col">
                          <span className="text-sm tabular-nums">
                            {format(deletedAt, "dd/MM/yyyy 'às' HH:mm", {
                              locale: ptBR,
                            })}
                          </span>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  ...
                                  dados associados a <strong>{p.name}</strong>{" "}
