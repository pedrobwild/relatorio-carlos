...
export function PaymentFlowModal({ open, onOpenChange, purchaseId, projectId, itemName }: PaymentFlowModalProps) {
  ...
  const updateRow = (id: string, field: keyof PaymentFlow, value: string | number) => {
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                ...
                <span>{paidCount}/{flows.length} concluídas</span>
              </div>
            ...
            {flows.length === 0 ? (
              <div className="py-8 text-center text-muted-foreground text-sm">
                Nenhuma parcela cadastrada. Clique em "Nova Parcela" para começar.
              </div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="min-w-[140px]">Data Pagamento</TableHead>
                    ...
                    <TableRow key={flow.id}>
                      <TableCell>
                        <Input
                          ...
                          onChange={(e) => updateRow(flow.id, 'installment_name', e.target.value)}
                        />
                      </TableCell>
                      <TableCell>
                        <Input
                          ...
                          value={flow.amount || ''}
                          onChange={(e) => updateRow(flow.id, 'amount', parseFloat(e.target.value) || 0)}
                        />
                      </TableCell>
                      <TableCell>
                        <Input
                          ...
                          className="h-8 text-sm"
                          value={flow.due_date || ''}
                          onChange={(e) => updateRow(flow.id, 'due_date', e.target.value)}
                        />
                      </TableCell>
                      <TableCell>
                        <Select value={flow.status} onValueChange={(v) => updateRow(flow.id, 'status', v)}>
                          <SelectTrigger className={cn(
                            ...
                            <div className="flex items-center gap-1.5">
                              {flow.status === 'concluido' ? <Check className="h-3.5 w-3.5" /> : <Clock className="h-3.5 w-3.5" />}
                              <span>{flow.status === 'concluido' ? 'Concluído' : 'Pendente'}</span>
                            </div>
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="pendente">
                              <div className="flex items-center gap-2"><Clock className="h-4 w-4" /> Pendente</div>
                            </SelectItem>
                            <SelectItem value="concluido">
                              <div className="flex items-center gap-2"><Check className="h-4 w-4" /> Concluído</div>
                            </SelectItem>
                          </SelectContent>
                        </Select>
                      </TableCell>
