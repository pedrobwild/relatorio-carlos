...
export function useComprasState(purchaseTypeFilter?: PurchaseType) {
  ...
  const [editingPurchase, setEditingPurchase] =
    useState<ProjectPurchase | null>(null);
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [formData, setFormData] =
    useState<Partial<PurchaseInput>>(emptyPurchase);
  ...
  const {
    ...
    onRestore: (draft) => {
      ...
      if (
        draft.paymentInstallments &&
        Array.isArray(draft.paymentInstallments)
      ) {
  ...
  const filteredPurchases = useMemo(() => {
    return purchases.filter((p) => {
      ...
      if (
        purchaseTypeFilter &&
        (p.purchase_type || "produto") !== purchaseTypeFilter
      )
        return false;

      if (filterStatus !== "all" && p.status !== filterStatus) return false;
      if (filterActivity !== "all" && p.activity_id !== filterActivity)
        return false;
