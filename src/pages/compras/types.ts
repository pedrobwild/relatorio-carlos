import { Clock, Package, Truck, CheckCircle2, X, ShieldCheck, ThumbsUp, ShoppingCart, ArrowRightCircle } from 'lucide-react';
...
export const statusConfig: Record<PurchaseStatus, { label: string; color: string; icon: React.ElementType }> = {
  ...
  awaiting_approval: { label: 'Solic. Aprovação', color: 'bg-orange-500/20 text-orange-600 border-orange-500/30', icon: ShieldCheck },
  approved: { label: 'Aprovado', color: 'bg-blue-500/20 text-blue-600 border-blue-500/30', icon: ThumbsUp },
  purchased: { label: 'Compra Realizada', color: 'bg-indigo-500/20 text-indigo-600 border-indigo-500/30', icon: ShoppingCart },
  ...
  delivered: { label: 'Entregue', color: 'bg-[hsl(var(--success))]/20 text-[hsl(var(--success))] border-[hsl(var(--success))]/30', icon: CheckCircle2 },
  sent_to_site: { label: 'Enviado p/ Obra', color: 'bg-emerald-500/20 text-emerald-600 border-emerald-500/30', icon: ArrowRightCircle },
