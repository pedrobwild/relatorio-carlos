...
import { Paperclip, X, FileIcon, ImageIcon, Loader2, Download } from 'lucide-react';
...
export function PurchaseAttachmentsField(props: Props) {
  ...
  const validateFiles = (files: FileList | File[]): File[] => {
    ...
    for (const f of Array.from(files)) {
      if (f.size > MAX_SIZE) {
        toast.error(`"${f.name}" excede 20 MB`, { description: 'Reduza o tamanho ou envie em outro formato.' });
        continue;
      }
      out.push(f);
    }
  ...
  const handleFiles = async (fileList: FileList | null) => {
    ...
    if (!user?.id) {
      toast.error('Sessão expirada', { description: 'Faça login novamente para anexar arquivos.' });
      return;
    }
    setBusyId('upload');
    try {
  ...
  const items: Array<{ key: string; name: string; size?: number | null; mime?: string | null; saved?: SavedAttachment; pendingId?: string }> = [
    ...(props.mode === 'pending'
      ? props.pending.map(p => ({ key: p.localId, name: p.file.name, size: p.file.size, mime: p.file.type, pendingId: p.localId }))
      : saved.map(s => ({ key: s.id, name: s.file_name, size: s.size_bytes, mime: s.mime_type, saved: s }))
    ),
  ];
  ...
  return (
    <div className="grid gap-2">
      <div className="flex items-center justify-between">
        <Label className="text-sm font-medium">{props.label ?? 'Anexos (opcional)'}</Label>
      ...
      >
        {busyId === 'upload' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Paperclip className="h-4 w-4" />}
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {it.size ? (
                    <p className="text-[11px] text-muted-foreground">{formatBytes(it.size)}</p>
                  ) : null}
                </div>
                {it.saved && (
                  <Button
                ...
                <Button
                  ...
                  onClick={() => it.saved ? removeSaved(it.saved) : removePending(it.pendingId!)}
                  disabled={busy}
