...
export function FornecedorSelector({ fornecedorId, onFornecedorChange, startDate, endDate, currentPurchaseId }: Props) {
  ...
  const { data: conflicts = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        ...
        .select('id, item_name, start_date, end_date, project_id, projects!inner(name)')
  ...
  const fmtDate = (d: string) => {
    try { return format(parseISO(d), 'dd/MM', { locale: ptBR }); }
    catch { return d; }
  };
        ...
        <div className="rounded-md border border-[hsl(var(--warning))]/40 bg-[hsl(var(--warning))]/10 p-2.5 space-y-1">
          ...
          {conflicts.map((c, i) => (
            <p key={i} className="text-[11px] text-muted-foreground">
              <span className="font-medium text-foreground">{c.project_name}</span>
