...
const schema = z.object({
  nome: z.string().trim().min(2, 'Nome obrigatório (mín. 2)').max(120, 'Máx. 120 caracteres'),
 ...
 */
export function QuickCreateFornecedor({ supplierType, onCreated, defaultName = '', invalidateKeys = [] }: Props) {
      ...
      <PopoverContent className="w-80 z-50" align="end">
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="qc-nome" className="text-xs">Nome *</Label>
            <Input
              id="qc-nome"
              value={nome}
              onChange={(e) => setNome(e.target.value)}
              placeholder={supplierType === 'prestador' ? 'Ex: João Pintor' : 'Ex: Casa do Construtor'}
              autoFocus
              maxLength={120}
            />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="qc-tel" className="text-xs">Telefone (opcional)</Label>
            <Input
              id="qc-tel"
              value={telefone}
