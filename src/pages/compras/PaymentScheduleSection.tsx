...
const PRESETS = [
  { label: '100% à vista', splits: [{ pct: 100, desc: 'Pagamento integral', dateRef: 'start' as const }] },
  { label: '50/50', splits: [
    { pct: 50, desc: 'Entrada (início)', dateRef: 'start' as const },
    { pct: 50, desc: 'Conclusão', dateRef: 'end' as const },
  ]},
  { label: '30/40/30', splits: [
...
function resolveDate(ref: 'start' | 'mid' | 'end', start: string, end: string): string {
...
export function PaymentScheduleSection({ totalValue, startDate, endDate, installments, onChange }: Props) {
  ...
  const addInstallment = () => {
    ...
    const remaining = 100 - installments.reduce((sum, i) => sum + (i.percentage || 0), 0);
    const remainingAmt = totalValue - installments.reduce((sum, i) => sum + (i.amount || 0), 0);
  ...
  const updateInstallment = (idx: number, field: keyof PaymentInstallment, value: string | number) => {
    setActivePreset(null);
    ...
    if (field === 'percentage') {
      ...
      inst.amount = amt;
      inst.percentage = totalValue > 0 ? Math.round((amt / totalValue) * 10000) / 100 : 0;
    } else {
      ...
      {installments.length > 0 && (
        <div className="space-y-2">
          {installments.map((inst, idx) => (
            <div key={idx} className="grid grid-cols-[1fr_80px_100px_110px_32px] gap-2 items-end">
              <div>
                {idx === 0 && <Label className="text-[11px] text-muted-foreground mb-1 block">Descrição</Label>}
                <Input
              ...
              <div>
                {idx === 0 && <Label className="text-[11px] text-muted-foreground mb-1 block">Vencimento</Label>}
                <Input
                  type="date"
                  value={inst.due_date}
          ...
          <div className={cn(
            ...
            isBalanced ? 'bg-[hsl(var(--success))]/10 text-[hsl(var(--success))]' : 'bg-[hsl(var(--warning))]/10 text-[hsl(var(--warning))]'
          )}>
            <span>Total: {totalPct.toFixed(1)}% — {fmt(totalAmt)}</span>
