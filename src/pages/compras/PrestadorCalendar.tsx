...
export function PrestadorCalendar({ onNew }: { onNew?: () => void } = {}) {
  ...
  const { data: entries = [], isLoading } = useQuery({
    queryKey: ['prestador-calendar', viewStart.toISOString(), viewEnd.toISOString()],
    queryFn: async () => {
  ...
  const projectColorMap = useMemo(() => {
    ...
    uniqueProjects.forEach((pid, i) => map.set(pid, PROJECT_COLORS[i % PROJECT_COLORS.length]));
    return map;
  }, [entries]);
      ...
      <div className="rounded-lg border bg-card overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b px-3 py-2 bg-muted/30">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">({grouped.length} prestadores)</span>
          </div>
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setWeekOffset(w => w - 1)}>
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="sm" className="h-7 text-xs px-2" onClick={() => setWeekOffset(0)}>
              Hoje
            </Button>
            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setWeekOffset(w => w + 1)}>
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[700px]">
            {/* Day headers */}
            <div className="grid border-b bg-muted/20" style={{ gridTemplateColumns: `140px repeat(${days.length}, 1fr)` }}>
            ...
            {grouped.map(([fornecedorId, { nome, entries: fEntries }]) => (
              ...
              >
                ...
                {days.map((day, dayIdx) => {
                  ...
                  const colorClass = projectColorMap.get(entry.project_id) || PROJECT_COLORS[0];
                    ...
                    <Tooltip key={dayIdx}>
                      <TooltipTrigger asChild>
                        <div className={cn(
                          ...
                          isToday && 'bg-primary/5',
                          activeEntries.length > 1 && 'ring-1 ring-[hsl(var(--warning))] ring-inset',
                        )}>
                          <div className={cn(
                            'h-6 w-full border-y',
                            colorClass,
                      ...
                      <TooltipContent side="top" className="max-w-[220px]">
                        {activeEntries.map((e, i) => (
                          <div key={i} className={cn('text-xs', i > 0 && 'mt-1 pt-1 border-t')}>
                            <p className="font-semibold">{e.fornecedor_nome}</p>
                            <p className="text-muted-foreground">{e.item_name}</p>
        ...
        <div className="border-t px-3 py-2 flex flex-wrap gap-3">
          {Array.from(projectColorMap.entries()).map(([pid, colorClass]) => {
            const projectName = entries.find(e => e.project_id === pid)?.project_name || '';
            return (
