...
import { Activity, Shield, Database, HardDrive, Link as LinkIcon, RefreshCw } from 'lucide-react';
...
export default function AdminHealth() {
  ...
  const runDiagnostics = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: 'Diagnóstico concluído', description: `Tempo total: ${results.totalLatencyMs}ms` });
    } catch (error) {
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <DiagnosticCard title="Autenticação" icon={Shield} result={state.auth}>
            {state.auth?.userId && (
          ...
          <DiagnosticCard title="Banco de Dados" icon={Database} result={state.db} />

          <DiagnosticCard title="Storage" icon={HardDrive} result={state.storage}>
          ...
          <DiagnosticCard title="Signed URLs" icon={LinkIcon} result={state.signedUrl} />
