...
export type PixKeyType = 'cpf' | 'cnpj' | 'email' | 'phone' | 'random' | 'copia_cola' | 'unknown';
...
export function validatePixKey(rawValue: string): PixValidationResult {
  ...
  if (!value) {
    return { valid: false, type: 'unknown', normalized: '', error: 'Chave PIX é obrigatória' };
  }
  ...
  switch (type) {
    case 'cpf': {
      const digits = onlyDigits(value);
      if (!isValidCPF(digits)) return { valid: false, type, normalized: digits, error: 'CPF inválido' };
      return { valid: true, type, normalized: digits };
    }
    case 'cnpj': {
      const digits = onlyDigits(value);
      if (!isValidCNPJ(digits)) return { valid: false, type, normalized: digits, error: 'CNPJ inválido' };
      return { valid: true, type, normalized: digits };
    }
    case 'email': {
      const normalized = value.toLowerCase();
      if (!isValidEmail(normalized)) {
        return { valid: false, type, normalized, error: 'E-mail inválido (máx 77 caracteres)' };
      }
    ...
    case 'random': {
      if (!isValidRandomKey(value)) {
        return { valid: false, type, normalized: value, error: 'Chave aleatória inválida (UUID esperado)' };
      }
    ...
    default:
      return {
        ...
        type: 'unknown',
        normalized: value,
        error: 'Formato não reconhecido. Use CPF, CNPJ, e-mail, telefone (+55…), chave aleatória ou Copia e Cola.',
      };
  }
}
