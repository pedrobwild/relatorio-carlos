...
describe('queryKeys', () => {
  describe('projects', () => {
    ...
    it('should return correct detail key', () => {
      ...
      expect(queryKeys.projects.detail(projectId)).toEqual(['projects', 'detail', projectId]);
    });

    it('should handle undefined project id', () => {
      expect(queryKeys.projects.detail(undefined)).toEqual(['projects', 'detail', undefined]);
    });
  });
  ...
  describe('documents', () => {
    ...
    it('should return correct list key with project id', () => {
      ...
      expect(queryKeys.documents.list(projectId)).toEqual(['documents', 'list', projectId]);
    });

    it('should return correct category key', () => {
      const projectId = 'project-123';
      const category = 'contrato';
      expect(queryKeys.documents.byCategory(projectId, category))
        .toEqual(['documents', 'list', projectId, 'category', category]);
    });

    it('should return correct versions key', () => {
      const docId = 'doc-123';
      expect(queryKeys.documents.versions(docId)).toEqual(['documents', 'versions', docId]);
    });
  });
  ...
  describe('activities', () => {
    it('should return correct list key', () => {
      const projectId = 'project-123';
      expect(queryKeys.activities.list(projectId)).toEqual(['activities', 'list', projectId]);
    });

    it('should return correct baseline key', () => {
      const projectId = 'project-123';
      expect(queryKeys.activities.baseline(projectId)).toEqual(['activities', 'baseline', projectId]);
    });
  });
  ...
  describe('formalizacoes', () => {
    ...
    it('should return correct list key with filters', () => {
      ...
      expect(queryKeys.formalizacoes.list(filters))
        .toEqual(['formalizacoes', 'list', filters]);
    });

    it('should return correct detail key', () => {
      const id = 'form-123';
      expect(queryKeys.formalizacoes.detail(id)).toEqual(['formalizacoes', 'detail', id]);
    });

    it('should return correct parties key', () => {
      const id = 'form-123';
      expect(queryKeys.formalizacoes.parties(id)).toEqual(['formalizacoes', 'parties', id]);
    });
  });
  ...
  describe('payments', () => {
    ...
    it('should return correct list key', () => {
      const projectId = 'project-123';
      expect(queryKeys.payments.list(projectId)).toEqual(['project-payments', 'list', projectId]);
    });
  });
  ...
  describe('pendingItems', () => {
    ...
    it('should return correct list key with options', () => {
      expect(queryKeys.pendingItems.list('project-1', true))
        .toEqual(['pending-items', 'list', 'project-1', true]);
    });

    it('should return correct stats key', () => {
      expect(queryKeys.pendingItems.stats('project-1'))
        .toEqual(['pending-items', 'stats', 'project-1']);
    });
  });

  describe('auditoria', () => {
    it('should return correct entity trail key', () => {
      expect(queryKeys.auditoria.entityTrail('documents', 'doc-123'))
        .toEqual(['audits', 'entity-trail', 'documents', 'doc-123']);
    });

    it('should return correct entity types key', () => {
      expect(queryKeys.auditoria.entityTypes()).toEqual(['audits', 'entity-types']);
    });
  });
  ...
  describe('users', () => {
    it('should return correct profile key', () => {
      ...
      expect(queryKeys.users.profile(userId)).toEqual(['users', 'profile', userId]);
    });
  ...
  describe('journey', () => {
    it('should return correct config key', () => {
      const projectId = 'project-123';
      expect(queryKeys.journey.config(projectId)).toEqual(['journey', 'config', projectId]);
    });

    it('should return correct slots key', () => {
      const stageId = 'stage-123';
      expect(queryKeys.journey.slots(stageId)).toEqual(['journey', 'slots', stageId]);
    });
  });
