...
describe('permissionGuard', () => {
  ...
  describe('hasModulePermission', () => {
    ...
    it('multi-role checks any', () => {
      expect(hasModulePermission(['customer', 'engineer'], 'works:write')).toBe(true);
    });
  ...
  describe('hasAllModulePermissions', () => {
    it('admin has all permissions', () => {
      expect(
        hasAllModulePermissions(['admin'], ['users:read', 'users:write', 'system:admin']),
      ).toBe(true);
    });
  ...
  describe('checkPermission', () => {
    it('admin bypasses all checks', () => {
      expect(
        checkPermission({ roles: ['admin'] }, 'users:delete'),
      ).toBe(true);
      expect(
        checkPermission({ roles: ['admin'], projectRole: 'viewer' }, 'obra:write'),
      ).toBe(true);
    });

    it('customer cannot write works', () => {
      expect(
        checkPermission({ roles: ['customer'] }, 'works:write'),
      ).toBe(false);
    });
