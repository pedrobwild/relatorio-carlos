...
describe('Common Validators', () => {
  ...
  describe('passwordSchema', () => {
    ...
    it('rejects short passwords', () => {
      expect(() => passwordSchema.parse('12345')).toThrow('mínimo 6 caracteres');
    });

    it('rejects long passwords', () => {
      const longPassword = 'a'.repeat(129);
      expect(() => passwordSchema.parse(longPassword)).toThrow('Senha muito longa');
    });
  });
...
describe('Project Schemas', () => {
  describe('createProjectSchema', () => {
    ...
    it('rejects invalid customer email', () => {
      expect(() =>
        createProjectSchema.parse({ ...validProject, customer_email: 'invalid' })
