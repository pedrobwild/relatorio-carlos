...
export async function logAudit(entry: AuditEntry): Promise<void> {
  try {
    ...
    if (error) {
      logError('Failed to write audit log', error, { component: 'auditLogger' });
    } else {
      logInfo('Audit logged', { component: 'auditLogger', ...entry });
    }
  } catch (err) {
    logError('Unexpected error in auditLogger', err, { component: 'auditLogger' });
  }
}
...
export const audit = {
  ...
  membershipChanged: (projectId: string, userId: string, action: 'create' | 'delete', role?: string) =>
    logAudit({
      action,
