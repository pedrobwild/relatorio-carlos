...
import type { ObraTask, ObraTaskStatus, ObraTaskPriority } from '@/hooks/useObraTasks';
...
export function deriveKanbanStatus(task: GlobalTask): KanbanStatus {
  ...
  if (task.status === 'em_andamento' || task.status === 'pausado') {
    if (task.due_date && task.due_date < new Date().toISOString().slice(0, 10)) return 'overdue';
    return 'in_progress';
  }
  if (task.due_date && task.due_date < new Date().toISOString().slice(0, 10)) return 'overdue';
  return 'not_started';
...
export function useAllActivities() {
  ...
  const updateStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: ObraTaskStatus }) => {
    ...
    onMutate: async ({ id, status }) => {
      ...
      if (prev) {
        queryClient.setQueryData<GlobalTask[]>(QUERY_KEY,
