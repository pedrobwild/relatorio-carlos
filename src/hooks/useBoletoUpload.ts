...
export function useBoletoUpload() {
  ...
  return useMutation({
    mutationFn: async ({ paymentId, projectId, file }: { paymentId: string; projectId: string; file: File }) => {
      ...
      logInfo('Uploading boleto', { paymentId, projectId, fileType: file.type, fileSize: file.size });
      ...
      if (uploadError) {
        logError('Boleto upload failed', uploadError, { component: 'useBoletoUpload', paymentId });
        throw uploadError;
      }
      ...
      if (updateError) {
        logError('Payment update failed after upload', updateError, { component: 'useBoletoUpload', paymentId });
        throw updateError;
      }
    ...
    onError: (error: Error & { code?: string }) => {
      logError("Error uploading boleto", error, { component: 'useBoletoUpload' });
