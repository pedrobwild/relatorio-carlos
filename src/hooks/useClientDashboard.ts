...
export function useClientDashboard() {
  const { user } = useAuth();
  const { data: projects = [], isLoading: projectsLoading, error: projectsError } = useProjectSummaryQuery();
  ...
  const { data: upcomingPayments = [] } = useQuery({
    ...
    queryFn: async (): Promise<UpcomingPayment[]> => {
      ...
      const { data, error } = await supabase
        ...
        .select('id, project_id, description, amount, due_date, installment_number')
