...
describe('useProjectNavigation', () => {
  ...
  it('should generate correct project paths', () => {
    ...
    expect(result.current.paths.financeiro).toBe('/obra/project-123/financeiro');
    expect(result.current.paths.pendencias).toBe('/obra/project-123/pendencias');
    expect(result.current.paths.documentos).toBe('/obra/project-123/documentos');
    expect(result.current.paths.formalizacoes).toBe('/obra/project-123/formalizacoes');
    expect(result.current.paths.formalizacoesNova).toBe('/obra/project-123/formalizacoes/nova');
  });
  ...
  it('should return path with projectId prefix when projectId exists', () => {
    ...
    expect(result.current.getProjectPath('/custom-path')).toBe('/obra/abc-123/custom-path');
  });
});
