...
export function useExecutivoVersions(projectId: string | undefined) {
  ...
  const createVersionMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      if (vErr) {
        if (vErr.code === '23505') throw new Error('Conflito de versão. Tente novamente.');
        throw vErr;
      }
      ...
      if (imgErr) {
        try { await supabase.storage.from(BUCKET).remove([storagePath]); } catch { /* ignore cleanup failure */ }
        try { await supabase.from('project_3d_versions').delete().eq('id', version.id); } catch { /* ignore cleanup failure */ }
        throw imgErr;
      }
