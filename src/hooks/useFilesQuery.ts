...
import type { FileFilters, CreateFileInput } from '@/infra/repositories/files.repository';
import { useAuth } from './useAuth';
import { toast } from 'sonner';

// Re-export types for convenience
export type { FileMetadata, FileWithUrl, FileStatus, FileVisibility } from '@/infra/repositories/files.repository';
...
export function useProjectFilesQuery(projectId: string | undefined) {
  return useFilesQuery(projectId ? { project_id: projectId, status: 'active' } : {});
}
...
export function useFileQuery(fileId: string | undefined) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Get signed URL
      const url = await filesRepo.getSignedUrl(result.data.bucket, result.data.storage_path);
    ...
    onSuccess: (_, variables) => {
      ...
      if (variables.metadata.project_id) {
        queryClient.invalidateQueries({
          queryKey: fileKeys.list({ project_id: variables.metadata.project_id }),
        });
      }
    },
    onError: (error: Error) => {
