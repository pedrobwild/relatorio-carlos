...
describe('supplierCategories — central taxonomy', () => {
  ...
  describe('SUPPLIER_SUBCATEGORIES_BY_TYPE', () => {
    it('every type has at least 10 subcategories', () => {
      for (const type of SUPPLIER_TYPES) {
        expect(SUPPLIER_SUBCATEGORIES_BY_TYPE[type].length).toBeGreaterThanOrEqual(10);
      }
    });
  ...
  describe('isValidSupplierSubcategory', () => {
    it('validates subcategory against correct type', () => {
      expect(isValidSupplierSubcategory('prestadores', 'Marcenaria')).toBe(true);
      expect(isValidSupplierSubcategory('produtos', 'Eletrodomésticos')).toBe(true);
    });

    it('rejects subcategory against wrong type', () => {
      expect(isValidSupplierSubcategory('prestadores', 'Eletrodomésticos')).toBe(false);
...
describe('supplierCategories — filter logic', () => {
  ...
  function filterPurchases(
    ...
    return purchases.filter(p => {
      if (filterCategory !== 'all' && p.category) {
        if (!isValidSupplierSubcategory(filterCategory, p.category)) return false;
  ...
  it('filters by category type (prestadores)', () => {
    ...
    expect(result.every(p => isValidSupplierSubcategory('prestadores', p.category!))).toBe(true);
  });
