-- 1) Add missing notification types for NCs
ALTER TYPE public.notification_type ADD VALUE IF NOT EXISTS 'nc_created';
ALTER TYPE public.notification_type ADD VALUE IF NOT EXISTS 'nc_status_changed';
ALTER TYPE public.notification_type ADD VALUE IF NOT EXISTS 'nc_pending_approval';
ALTER TYPE public.notification_type ADD VALUE IF NOT EXISTS 'nc_assigned';

-- 2) Drop duplicate trigger
DROP TRIGGER IF EXISTS trg_notify_nc_changes ON public.non_conformities;

-- 3) Update notify_nc_changes to use correct URL path
...
BEGIN
  ...
  v_status := NEW.status;
  v_action_url := '/obra/' || NEW.project_id || '?tab=ncs';
