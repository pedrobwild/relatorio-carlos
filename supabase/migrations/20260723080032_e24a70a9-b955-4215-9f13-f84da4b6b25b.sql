-- Onda gerencial: snapshot batch de portfólio para o Painel de Obras (staff-only).
-- Consolida avanço físico (Onda A), custos/EAC (Onda B), NCs, punch e lookahead
-- em UMA única RPC, evitando N queries por linha na tabela.

CREATE OR REPLACE FUNCTION public.get_portfolio_management_snapshot()
RETURNS TABLE (
  project_id uuid,
...
COMMENT ON FUNCTION public.get_portfolio_management_snapshot() IS
  'Snapshot batch (staff-only) do portfólio de obras ativas. Consolida avanço físico ponderado, custos/EAC, NCs, punch list e lookahead 14d numa única chamada, para o Painel de Obras.';