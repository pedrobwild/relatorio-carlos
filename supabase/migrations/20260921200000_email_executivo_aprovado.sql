-- E-mail imediato para Bianca e Mariana quando uma obra entra em "Executivo Aprovado".
--
-- Contexto: a versao anterior (20260831190000) so inseria a notificacao do sino
-- para a Mariana e contava com o resumo diario por e-mail
-- (send-notification-digest), que nunca foi agendado no pg_cron. Resultado:
-- nenhum e-mail chegava. Agora o trigger:
--   1. insere a notificacao do sino para CADA destinatario da lista;
--   2. chama a edge function send-executivo-aprovado via pg_net, que envia o
--      e-mail na hora (Resend) apenas para perfis ativos de users_profile.
--
-- Para incluir/remover pessoas, altere APENAS a lista c_destinatarios.
-- A seguranca operacional continua a mesma: nada aqui pode impedir a
-- movimentacao do card — todo o corpo esta em bloco EXCEPTION com WARNING.
  ...
  IF NEW.painel_etapa IS DISTINCT FROM OLD.painel_etapa
    ...
    BEGIN
      ...
      IF array_length(v_user_ids, 1) > 0 THEN
        -- Assincrono: a resposta fica em net._http_response para auditoria.
